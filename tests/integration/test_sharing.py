"""
test_sharing.py — integration tests for TASK-094 (server-side sharing).
...
attribution, and the anonymous-scoped-session restrictions. This is
TASK-094's own validation, not the full scenario matrix — TASK-097 (after
TASK-095 lands a real client) is where the comprehensive sharing/E2EE
regression suite belongs.
...
from vw_client import VwClient, VwProtocolError, VwAuthError, VW_PERM_VIEW, VW_PERM_EDIT
...
def test_scoped_session_cannot_grant_or_create_link(server, admin_client, unique_username):
    ...
    try:
        ...
        with pytest.raises(VwProtocolError):
            anon.share_grant(info["session_token"], fid, f"{unique_username}_owner", VW_PERM_VIEW)
        with pytest.raises(VwProtocolError):
