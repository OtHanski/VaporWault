...
int main(int argc, char **argv) {
    ...
    /* ── Anchor directory: vault_setup's folder_file_id must be a real
     * VW_ENTRY_DIR the caller owns — handle_file_commit's "file_id names a
     * directory -> create a new child file under it" branch (which
     * vw_vault_upload_file's file_id==0 path relies on) explicitly checks
     * entry_type == VW_ENTRY_DIR (see src/server/vw_file_handlers.c);
     * VAULT_CREATE itself is more lenient (any owned file_id), but that
     * leniency doesn't extend to FILE_COMMIT. FILE_MKDIR (TASK-104) is the
     * only wire mechanism that produces a real directory. */
    uint64_t folder_id = 0;
    err = vw_client_file_mkdir(sess, 0, "vault_dir", &folder_id);
    CHECK(err == VW_OK, "mkdir vault_dir");
    CHECK(folder_id != 0, "vault_dir has a nonzero file_id");
