...
`TASK-00169`'s design requires the replica pull loop (`vw_cluster.c`) to
...
published in `docs/PROTOCOL.md` before `TASK-00172` implements it.
...
Free message-type range confirmed during `TASK-00169`'s research:
`0x0708`–`0x07FE`.
...
- `CLUSTER_CHUNK_FETCH` (`0x070E`) / `CLUSTER_CHUNK_DATA` (`0x070F`):
  ...
  `TASK-00171`'s GC-gating fix for why).
...
`TASK-00172`'s own "Work" section was updated to match before any code was
...
**Correction [2026-08-14], found starting `TASK-00172`'s implementation:**
