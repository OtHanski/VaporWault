...
`docs/PROTOCOL.md` §7.11 (`TASK-00089`). The server's role here is
...
- Extend `vw_version_record_t` to carry `vault_id` + a reference (offset/
  ...
  CQR.08 on-disk-compatibility review, matching how `TASK-00090`'s
...
- Confirm dedup requires no code changes (§7.11.2) — encrypted chunks
  ...
  `TASK-00099`) isn't actually producing unique ciphertext and should be
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00089` once its
...
quota flake documented in `TASK-00095`'s notes. MSVC build of
...
— `docs/PROTOCOL.md` §7.11.4 documents this as a known gap for `TASK-00099`
...
- Dedup requires no code changes, confirmed by inspection of
  ...
  uniqueness follows from `TASK-00099`'s future per-file DEK generation, not
...
compatibility claim specifically (matching `TASK-00090`'s `deleted_at`
...
findings. Closing as done. `TASK-00101` (E2EE regression tests) may proceed
once `TASK-00099` (client vault module) exists — both remain open. `TASK-00099`
...
QA.06 [2026-07-31]: `TASK-00101`'s regression suite
