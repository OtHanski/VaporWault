...
Discovered while implementing the web gateway's vault UI (`TASK-00141`) —
...
and (TASK-00109's later addition) `version_id` — no `vault_id` field exists
...
- The web gateway's frontend (`TASK-00141`) worked around this by keying
...
WEB.09 [2026-08-12]: Filed while implementing `TASK-00141`'s vault UI —
...
`TASK-00141`'s own implementation note for the workaround's reasoning and
its limits.
...
once: revision 14 (`TASK-00100`, GUI.03) added `vault_id` to
...
gateway's own folder-level-only workaround (`TASK-00141`) is a second,
...
- `src/server/vw_file_handlers.c`'s `handle_file_list` — a second
  ...
  the existing `version_id` array (`TASK-00109`'s precedent — see that
...
- Filed `TASK-00158` (GUI.03) and `TASK-00159` (WEB.09) — out-of-domain
...
`TASK-00141`'s folder-level workaround) are real, independently-motivated
...
applied consistently with `TASK-00109`'s existing precedent, the
...
claimed. Filing `TASK-00158`/`TASK-00159` rather than reaching into GUI/web
