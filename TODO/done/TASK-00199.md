...
**Correction (2026-08-26)**: `TASK-00197`'s published wire spec (§7.12) has
...
- `vw_client_core.h`/`.c`: new `vw_search_entry_t` + `vw_client_search()`,
  ...
  layer, matching `TASK-00197`'s actual wire shape.
...
- New `tests/integration/test_cli_search.py`, driving the real compiled
  ...
  proven once at the wire level by `TASK-00198`'s `test_search.c` — this
...
- **Found and fixed a real test-infra issue while first running this**:
  ...
  `TASK-00198`'s short-lived test-harness sessions that could just be
...
- **Found and filed a real, separate product gap while first writing
  ...
  existing directory-creation path is the reverse, `TASK-00113`'s
  ...
  leaf-level operation, not a recursive-create). Filed as `TASK-00218`
...
(includes both `test_cli_search.py` and `TASK-00198`'s `test_search.py`).
...
- The two discoveries during implementation (test-server
  `max_workers` headroom; `TASK-00218`'s new-local-subdirectory sync gap)
