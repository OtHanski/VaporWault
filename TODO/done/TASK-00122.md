...
priority:    normal
depends_on:  []
blocks:      [TASK-00123, TASK-00124]
review_by:   [SEC.07, CQR.08]
tags:        [protocol, server, gui, audit, security-sensitive]
---

Discovered by CQR.08 review of TASK-00116 (audit log filter/search/export in
...
TASK-00116's audit-log GUI originally treated all `FILE_WRITE` entries as
...
- Give `FILE_WRITE` oplog entries a way to be disambiguated — either a
  ...
  TASK-00121 will also need).
- Tag `security-sensitive` and route through SEC.07 for the same reason as
  TASK-00121: this touches a format cluster replication
...
- Consider doing this alongside TASK-00121 (oplog timestamp) if a version
...
CQR.08 [2026-08-04]: Filed from a TASK-00116 review finding. Normal priority
(not low, unlike TASK-00121) — this isn't just a missing nicety, it's the
...
§7.7, landed together with TASK-00121's timestamp field as one migration.
...
TASK-00123 (SRV.01, the three call sites + oplog internals) and TASK-00124
(GUI.03, restores owner-id attribution for the create case), both blocked
on this task and TASK-00121 reaching `done`.

SEC.07 [2026-08-04]: Review finding (**blocking**), filed identically on
`TASK-00121` since it's the same spec — see that task's SEC.07 note for full
detail: (1) the draft's `ts_unix_ms` wording implied replicas re-stamp the
timestamp on apply rather than preserving the primary's, and (2) `TASK-00123`
...
PRT.04 [2026-08-04]: Resolved — see `TASK-00121`'s matching PRT.04 note.
§7.7 corrected and `TASK-00123` now explicitly lists `vw_cluster.c`'s two
...
collisions with existing or reserved values. Sign off — see `TASK-00121`'s
CQR.08 note for the shared spec-level detail.

ARCH.00 [2026-08-04]: All `review_by` sign-offs recorded, no unresolved
blocking findings. Marking `done`; implementation tracked in `TASK-00123`/
`TASK-00124`.
