...
blocks:      [TASK-00137, TASK-00138, TASK-00139, TASK-00140, TASK-00141]
review_by:   [CQR.08]
tags:        [web, build]
---

Scaffold the static frontend per `TASK-00127`'s "plain TypeScript, no SPA
...
2. A minimal `index.html` shell and a small client-side router (hand-rolled
   — no framework) for the views `TASK-00137`–`TASK-00141` will add.
3. A `fetch()`-based API client module wrapping the gateway's `/api/*`
   endpoints (`TASK-00132`–`TASK-00135`), including session-cookie handling.
4. An example nginx config: serves `web/dist/` (or equivalent build output)
   as static assets, reverse-proxies `/api/*` to
   `vapourwault-web-gateway`'s loopback HTTP listener (`TASK-00129`). Add this
   to `docs/DEPLOYMENT.md` alongside `TASK-00142`'s packaging work (coordinate
...
- Router + API client scaffold is in place for `TASK-00137`–`TASK-00141` to add
...
`TASK-00137`'s note). `web/node_modules/`, `web/dist/` gitignored (build
...
(not just TypeScript type-checking) — see `TASK-00137`/`TASK-00138`'s note for
...
in `main.ts` (`TASK-00137`/`TASK-00138`) is therefore unverified in an actual
