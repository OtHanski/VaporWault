...
created:     2026-08-14
priority:    high
depends_on:  [TASK-00169]
blocks:      [TASK-00172, TASK-00178]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, server]
---

Found during `TASK-00169`'s design research, independent of the replication
...
`TASK-00172` makes replicas genuinely fetch chunk content, a lagging replica
...
Fix this BEFORE `TASK-00172` lands (it's `TASK-00172`'s own dependency,
...
- With a replica configured and deliberately lagging (test harness pulls
  ...
  test (`TASK-00178`), not just code inspection.
...
adding a new one — it already supports exactly the scenario `TASK-00178`
...
`TASK-00178`'s job per this task's own acceptance criteria, since it needs
`TASK-00172` to exist first to be a meaningful end-to-end test rather than
...
criteria deferred to `TASK-00178`: real primary + real replica, a real
...
`handle_file_commit`'s `addref` (filed as `TASK-00180`, SRV.01) — the test
...
`TASK-00180` lands, rather than silently staying green either way.
