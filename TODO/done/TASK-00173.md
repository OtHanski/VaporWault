...
blocks:      [TASK-00174, TASK-00175, TASK-00178]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, client]
---

Depends on `TASK-00172`'s replica hot-standby (a fallback target must have
real, live data now). Each `vw_account_ctx_t`/`account.conf`
(`TASK-00161`'s per-account, per-server model) gains an OPTIONAL fallback
...
safe given `TASK-00172`'s replication actually keeps the replica's own user
...
- `vw_account_cfg_t`/`account.conf` (`vw_daemon.c`): added `fallback_host`/
  ...
  rather than inventing a second one (TASK-00174 can revisit this if a
...
- `STATUS_RESP` gained a trailing `any_on_fallback` byte;
  ...
  documented in `vw_ipc.h`. Real UI surfacing of these is `TASK-00174`/`00175`'s
...
- **Process note for future work in this repo:** `build-gw-e2e`'s
  ...
  passed" claim earlier in this same session (during TASK-00171/00172) was
...
- Out-of-domain discovery opened as `TASK-00179` (SRV.01): the server's own
...
- Not done here, left to later tasks in this milestone: CLI surfacing of
  fallback config/status (`TASK-00174`), GUI surfacing (`TASK-00175`), the
  ...
  the full stack exists — `TASK-00178`).

**QA.06, 2026-08-17 — TASK-00178 sign-off.** `tests/integration/
test_cli_fallback.py::test_cli_account_add_fallback_and_real_failover`
now additionally verifies (beyond what `TASK-00174` already covered): a
