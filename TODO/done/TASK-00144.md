...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127, TASK-00131, TASK-00132, TASK-00133, TASK-00134, TASK-00135, TASK-00141]
blocks:      []
review_by:   [CQR.08]
...
(`TASK-00127`), beyond the per-task `review_by` sign-offs each implementation
task already requires. This is the feature-level pass `CLAUDE.md`'s Step 6
calls for, matching the depth `TASK-00089`'s design review and `TASK-00085`'s
independent re-verification got for the vault feature.

Specific items to check, carried forward from `TASK-00127`'s design and the
individual implementation tasks' own flagged risks:

1. **New externally-reachable attack surface**: the gateway's HTTP/JSON
   layer (`TASK-00129`/`TASK-00130`) is the first-ever HTTP-facing code in this
...
2. **Session/auth handling** (`TASK-00131`/`TASK-00132`): cookie attributes
...
3. **XSS/CSRF**: filename rendering (`TASK-00138`), public link token
   handling (`TASK-00140`), and whether gateway state-changing endpoints need
   CSRF protection given cookie-based auth.
4. **Path traversal / cross-user access** (`TASK-00133`/`TASK-00134`): confirm
   the gateway introduces no new traversal vector on top of the server's
   existing owner-namespaced checks.
5. **Vault zero-knowledge boundary** (`TASK-00135`/`TASK-00141`): the single
   highest-stakes property in this feature — confirm by code inspection
   (not just `TASK-00143`'s test) that no gateway code path can ever receive
   ...
   compatibility bug).
6. **Session-storage precedent** (`TASK-00131`): confirm the gateway did not
...
Findings tagged `blocking` prevent any `TASK-00128`–`TASK-00141` task (or
`TASK-00127` itself) from moving to `done`, per `CLAUDE.md`'s routing rules.
...
- Multi-session isolation: structurally sound by construction — each
  ...
  slots. Live two-concurrent-session verification is `TASK-00143`'s own
  required coverage item, deliberately not duplicated here — this note
  covers *why* it should hold, `TASK-00143` proves it does.
...
- CSRF: `SameSite=Strict` cookies are never sent on any cross-site
  ...
  execution in-origin).
- Link token handling: already covered in depth in `TASK-00140`'s own note
...
field in any of `TASK-00135`/`00141`'s JSON schemas to begin with. The
...
`TASK-00141` (not just asserted) — re-read those three verification
...
**Additional note for `TASK-00142`'s docs**: deployment guidance should say
...
`review_by`. Both blocking findings are fixed and re-verified; `TASK-00143`
...
answer with a code reference, not a bare "looks fine." `TASK-00143`'s
...
sign-offs, same as how `TASK-00089`'s design review and the vault
