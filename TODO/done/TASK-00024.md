...
created:     2026-07-10
priority:    high
depends_on:  [TASK-00021, TASK-00022, TASK-00023]
blocks:      []
...
Each handler validates the session token, enforces path rules from TASK-00021 §6, calls
`vw_store` (TASK-00022) and `vw_storage_files` (TASK-00023), and sends the appropriate
response message.

TASK-00021 (protocol spec), TASK-00022 (file/version store), and TASK-00023 (chunk store)
...
2. Calls `vw_store_session_get` (from TASK-00008) to look up the session.
...
- Decodes the payload using the framing from TASK-00021 §2.
...
- Use `vw_store_user_update` (from TASK-00008) to persist the updated quota.

### 5. Dispatch integration

Register all new handlers in the existing `vw_server_core` message dispatch table (the
`switch` or handler-array introduced in TASK-00011). No new connection or threading changes
are needed.
...
1. **`vw_path_validate`**: enforces all six TASK-00021 §6 path rules in a single pass.
...
**Deviations from TASK-00024 spec:**
...
ARCH.00 [2026-07-11]: TASK-00024 closed. CQR.08 and SEC.07 have signed off with no
blocking findings. Phase 2 file-transfer request handlers are complete.

Phase 2 milestone summary:
- TASK-00021 (protocol spec v4): done
- TASK-00022 (file/version store): done
- TASK-00023 (chunk content store): done
- TASK-00024 (server request handlers): done
- TASK-00025 (client sync engine): done
