...
created:     2026-07-29
priority:    high
depends_on:  [TASK-00088, TASK-00094]
blocks:      [TASK-00096, TASK-00097]
review_by:   [SEC.07, CQR.08]
tags:        [client, protocol, security-sensitive]
---

Implement the client-side API for the sharing design published in
`docs/PROTOCOL.md` §7.5 (`TASK-00088`). GUI.03 blocks on this per the existing
...
- Client library functions for every message in §7.5 exist and are
  ...
  (once TASK-00094 lands).
- CLI commands work end-to-end.
- Integration point definition for GUI.03 is written down (in this task's
  notes or a doc) before GUI.03 starts `TASK-00096`.

## Notes

<!-- Agents append notes below with their ID and date. Do not delete prior notes. -->

ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00088`. Depends on
`TASK-00094` for the real wire handlers to test against, but the client-side
...
sync-engine tree-integration item — filed separately as `TASK-00106` (see its
...
see rationale in `TASK-00105`'s notes for when out-of-domain filing applies
...
`tests/integration/test_sharing.py` (`TASK-00094`).
...
relay for the server's already-reviewed (`TASK-00094`) anti-enumeration
...
findings. Closing as done. `TASK-00096` (GUI sharing) may now proceed against
...
(`TASK-00106`), so a shared-item browser view should drive `SHARE_LIST`/
`LINK_LIST`/file-id operations directly rather than expecting them to
appear via `add-folder`/`ls`.

QA.06 [2026-07-31]: `TASK-00097` closed — its regression matrix exercises
