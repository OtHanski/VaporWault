...
created:     2026-08-31
priority:    high
depends_on:  [TASK-00226, TASK-00227, TASK-00228]
blocks:      [TASK-00231, TASK-00232]
review_by:   [CQR.08]
tags:        []
---
...
- Login activity (username/password, 2FA challenge/response via `VwClient`),
  wired to TASK-00227's account registry (add/switch/remove profile).
- File browser: RecyclerView-based list + breadcrumb navigation, backed by
  ...
  file-op actions (delete/move/rename via the ops added in TASK-00226).
- Transfer queue screen: shows in-flight/completed uploads/downloads from
  TASK-00228's transfer service, with progress and cancel.
...
(TASK-00230/00231/00232) — this task is the base browse/login/transfer experience
...
same `TASK-00237` gap hit again while setting up this test) plus an `aiosmtpd`
...
2. Diagnosed but **not fixed here** — out of MOB.10's domain: a genuine
   ...
   *every* `AUTH_REQUEST` for a 2FA-enabled account, with no reuse of a
   ...
   unreachable via the UI as designed. Filed as `TASK-00238` (SRV.01,
   ...
   blocked on `TASK-00238`, not on anything in this task's own scope.
...
surfaces are TASK-00230's scope), so `SEC.07` is not required here.
