...
created:     2026-08-11
priority:    high
depends_on:  [TASK-00146, TASK-00147, TASK-00148, TASK-00149, TASK-00150]
blocks:      []
...
1. Pass the real version into CMake: `-DVW_VERSION=${TAG#v}` (strip the
   ...
   directly into a shell script" discipline `TASK-00081` established — this
...
3. `build-windows`: add a step to fetch/install WiX Toolset v3
   ...
   verification of `TASK-00147`). After configure/build, run
   `cpack -G WIX` once per component (`server`, `client` — each has its
   own `CPACK_WIX_UPGRADE_GUID` per `TASK-00147`/`TASK-00148`) and stage both
   MSIs for upload.
...
5. Update the smoke-test step's rationale comment if the new artifacts
   ...
   workflow that package *installability* is `TASK-00152`'s manual/VM-based
...
- No ref-controlled value (`github.ref_name`, `github.event.inputs.version`)
  ...
  through workflow-level `env:` only, matching `TASK-00081`'s established
  pattern.
...
BLD.05 [2026-08-11]: While implementing `TASK-00147` in this session,
...
and is what `TASK-00147`'s verification actually used. Whoever implements
...
in `TASK-00147`'s implementation note.
...
expression directly into a shell/PowerShell script" discipline `TASK-00081`
...
- `ubuntu:24.04` with `dpkg-dev` + `rpm` installed: confirmed `rpmbuild` is
  ...
  package, and `TASK-00150`'s own RPM testing had only used a Fedora
...
correctness `TASK-00153` already verified. Confirmed by reading every
...
routes through job-level `env:` first, matching the `TASK-00081`
