...
`docs/TUTORIAL.md` (see that doc's final section and `TASK-00085`'s
...
accordingly. Filed `TASK-00093` for a real fix (`/proc/net/tcp`-based
...
1. Flagged that removing the broken SO_PEERCRED check, while a net
   ...
   host. Recommended elevating `TASK-00093`'s priority and documenting the
   risk. Done: `TASK-00093` bumped to high priority; added an explicit
