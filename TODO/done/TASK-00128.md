...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127]
blocks:      [TASK-00129, TASK-00130, TASK-00136, TASK-00142]
review_by:   [CQR.08]
tags:        [build, gateway]
---

Add the build scaffolding for the new web gateway component designed in
`TASK-00127`. This task is build wiring only — no gateway logic (that's
`TASK-00129`–`TASK-00135`).
...
5. Leave `src/gateway/main.c` as a stub that links and runs (prints version,
   ...
   Real logic lands in `TASK-00129`–`TASK-00135`.
...
- No new external dependency introduced by this task (that decision is
  `TASK-00129`/`TASK-00130`'s hand-rolled-vs-vendor call, not this one's).
...
(`OPTIONAL` — the unit file itself is `TASK-00142`'s job, not filed yet).
...
primitives" for the gateway generally — that line applied to `TASK-00129`'s
scope, not this one; `TASK-00129` corrects it directly (`vw_net.h` is
...
isn't required for this task specifically, though `TASK-00144`'s
...
elsewhere (`TASK-00144`'s single-threaded-architecture note). No findings.
