...
blocks:      [TASK-00177, TASK-00178]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, gateway]
---

Depends on `TASK-00172`'s replica hot-standby. Same "opt-in read-only
fallback" idea as `TASK-00173`, at the gateway level instead of per-account
...
- New `main.c` flags: `--fallback-server-host`, `--fallback-server-port`,
  ...
  `TASK-00174`). Optional — omit for today's unchanged behavior.
- Connection logic: `handle_login`/every `require_session`-gated handler's
  ...
  "gateway has no persistent state of its own beyond `TASK-00165`'s
  remember-me store" posture; do not invent one here.
- `/api/accounts`-style status: the frontend needs a way to know "this
  session is on the read-only fallback" so `TASK-00166`'s existing
...
Same second-trust-anchor reasoning as `TASK-00173`'s security note, at the
...
- `main.c`: `--fallback-server-host`/`--fallback-server-port`/
  ...
  (matching `TASK-00174`'s CLI convention) before the gateway even starts.
...
- Connection logic (`vw_gateway_api.c`): new `is_net_err`/
  ...
  Unlike the daemon (`TASK-00173`), the gateway needs no persisted
  ...
  meaningless against a different server, same reasoning `TASK-00173`
...
- `/api/accounts`'s existing per-slot response gained a `read_only`
  ...
  GUI's own `conn_mode` surfacing (`TASK-00173`/`00175`). No frontend
  (TypeScript) work done here — reading this field is `TASK-00166`'s
...
**QA.06, 2026-08-17 — TASK-00178 sign-off.**
