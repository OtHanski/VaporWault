...
VERSION_LIST / VERSION_RESTORE operations from the Phase 2 wire protocol (TASK-00021).
TASK-00021 must be done first; this task can run in parallel with TASK-00022, TASK-00023,
and TASK-00024.

The client does not implement the sync engine (`vw_sync`, TASK-00026 / Phase 3). It
...
All file transfer functions take a `vw_client_sess_t *sess` (from TASK-00011). They must
...
1. Send FILE_STAT to get the current version's chunk list... wait — the server does not
   ...
   type to get the chunk list for a version. **This is a gap in TASK-00021** — note it as a
   question to PRT.04 and add a `VW_MSG_VERSION_CHUNKS` request/response pair that
   returns the ordered chunk hash list for a given version_id. Block on TASK-00021 update
   before implementing this step.

   Workaround until TASK-00021 is updated: include the chunk list in FILE_STAT_RESP as a
...
as TASK-00021 §6. Return `VW_ERR_PATH_INVALID` locally without making a network call.
...
ambiguity. CLI.02 must flag it in the TASK-00021 body as a question to PRT.04 before
implementing the download pipeline. Do not guess — add the note and wait for PRT.04's
answer in TASK-00021.
...
**SEC.07 advisory SEC.07-A-1 (from TASK-00021) compliance**: download path returns
...
**SEC.07 advisory SEC.07-A-3 (from TASK-00021) compliance**: CHUNK_QUERY_RESP with
...
**SEC.07-A-1 (from TASK-00021, carried)**: `recv_expect` passes server error codes
...
**SEC.07-A-2 (from TASK-00021, carried)**: `sess_check_valid` is the first call in
every entry point before any payload is constructed or sent. No payload fields are
processed before the session check. ✓

**SEC.07-A-3 (from TASK-00021, carried)**: count=0 CHUNK_QUERY_RESP bitmask: the
...
(TASK-00024) reimplements it. Low priority — mark for Phase 3 refactor.
...
Sign-off: implementation is correct and consistent with TASK-00021 spec. TASK-00025 may
move to done.

---

ARCH.00 [2026-07-11]: TASK-00025 closed. SEC.07 and CQR.08 signed off; no blocking
findings. Client-side Phase 2 file transfer API is complete. Advisories routed to
Phase 3 backlog. TASK-00022 result still pending; TASK-00023 and TASK-00024 remain blocked.
