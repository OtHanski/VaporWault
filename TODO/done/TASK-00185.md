...
priority:    normal
depends_on:  []
blocks:      [TASK-00186, TASK-00187, TASK-00188, TASK-00189, TASK-00190, TASK-00191]
review_by:   [CQR.08]
tags:        [design]
---
...
4. **Scope**: both fields optional and independent — a link can have
   ...
   as `TASK-00106`'s `FILE_LIST` `dir_file_id` extension — no protocol
   version bump.
...
|------|----------|------------|-----------|------|
| `TASK-00186` — Protocol: `LINK_CREATE`/`LINK_ACCESS` password field additions, new error codes, in `docs/PROTOCOL.md` | PRT.04 | 185 | CQR.08 | protocol |
| `TASK-00187` — Server: password storage + enforcement (Argon2id hash; brute-force mitigation via the existing `LINK_ACCESS` IP rate limiter, no new lockout mechanism) | SRV.01 | 186 | SEC.07, CQR.08 | security-sensitive, server |
| `TASK-00188` — Client: daemon IPC + client-core wrapper + `vapourwault-cli link create --password` | CLI.02 | 187 | CQR.08 | client |
| `TASK-00189` — GUI: password field in the "Create link" dialog, badge in the link list | GUI.03 | 188 | CQR.08 | gui |
| `TASK-00190` — Web gateway + frontend: create-link form gains both an expiry picker (wiring up the gateway API's already-existing `expiresAt` parameter, `web/src/api.ts:396`, which the form never surfaced) and a password field; password-prompt on redemption | WEB.09 | 187 | SEC.07, CQR.08 | security-sensitive, gateway |
| `TASK-00191` — Integration tests: password (missing/wrong/correct), brute-force rate limiting, regression on existing (password-less) links | QA.06 | 186-190 | CQR.08 | test |
...
CLAUDE.md's routing rule 3 — `TASK-00186` (protocol) must land before
`TASK-00187`/`00188` pick up implementation.

PRT.04 [2026-08-26] — **correction, found while starting `TASK-00186`**:
...
wired a picker to it (real, kept as part of `TASK-00190`). The original
...
password protection only (decisions 2-5), and `TASK-00186`-`191` below
