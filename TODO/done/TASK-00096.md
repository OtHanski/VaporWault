...
created:     2026-07-29
priority:    normal
depends_on:  [TASK-00095, TASK-00107, TASK-00108]
blocks:      []
review_by:   [CQR.08]
tags:        [gui]
---

Add GUI surfaces for the sharing feature designed in `TASK-00088` /
`docs/PROTOCOL.md` §7.5, once CLI.02 (`TASK-00095`) has published the client
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00088`. Blocked on
`TASK-00095` per the standing GUI.03 constraint — do not start until CLI.02's
client API for sharing is marked done.

ARCH.00 [2026-07-31]: `TASK-00095` is done, but starting this task surfaced a
...
`TASK-00107` (login) and `TASK-00108` (browser) as blocking prerequisites per
...
Scope note for whoever picks this up: `TASK-00108`'s browser shows the
...
payload has no `file_id` field, per `TASK-00095`'s own header comment on
...
`VW_IPC_LINK_*` messages from `TASK-00095`), returning structured
...
reasoning as `TASK-00104`'s note on when that's appropriate):**
...
`TASK-00107`/`TASK-00108`, not committed) to also exercise `share_grant` →
...
IT-7 quota flake noted in `TASK-00095`'s notes). MSVC build of
...
Same disclosed limitation as `TASK-00107`/`TASK-00108`: no input-injection or
