...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127, TASK-00135, TASK-00138]
blocks:      []
...
(`TASK-00127`'s decision, `ARCHITECTURE.md`'s E2EE model from `TASK-00089`):
...
opaque ciphertext and wrapped-key blobs (`TASK-00135`).
...
1. **Argon2id in the browser**: compile the project's existing vendored
   ...
   `parallelism = 1`, per `TASK-00089`'s design) consistent across native and
...
3. Key hierarchy logic in TypeScript: derive KEK from passphrase +
   ...
   `TASK-00135`'s `VAULT_KEY_FETCH` passthrough), derive/unwrap per-file DEKs,
   decrypt downloaded ciphertext / encrypt before upload. Match the nonce
   scheme settled in `docs/PROTOCOL.md` §7.11 (`TASK-00089`'s design:
...
4. Vault UI: create/unlock/browse flows, plus the same disclosures the
   native GUI already requires (`TASK-00100`'s acceptance criteria) —
   ...
   per `TASK-00089`'s accepted tradeoff in §7.11.5) — do not silently drop any
...
- Two different files in the same vault with identical plaintext produce
  ...
  `TASK-00101`'s equivalent native-client test requirement from `TASK-00089`'s
  SEC.07 review).
...
1. **Argon2id in WASM** (`web/wasm/`): `vw_vault_kdf_wasm.c` wraps
   ...
   *output* (`web/wasm/dist/*.js/.wasm`) is committed like any other
   ...
   `TASK-00142` to weigh in on long-term).
...
3. **Gateway extension**: `/api/files/commit` (`TASK-00133`/`00139`) gained
...
4. **`web/src/api.ts`/`main.ts`**: vault registry wrappers
   ...
   path, TASK-00139, but every chunk is encrypted/decrypted client-side
   ...
   delta-sync/bandwidth, matching `TASK-00100`'s native-GUI equivalent
...
- `VW_ERR_ALREADY_EXISTS` (e.g. `mkdir` on a name that already exists)
  ...
  (`TASK-00133`'s helper) and fell into the same "unrecognized error →
  evict + 500" catch-all built for `TASK-00155` — exactly the same class
  of gap as the `VW_ERR_AUTH_REQUIRED` fix from `TASK-00134`'s testing.
...
- Filed `TASK-00156` (assigned `PRT.04`, out of this role's domain): a
...
other frontend task this session (`TASK-00136`-`140`). Every piece of
...
same patterns already used (and flagged) in `TASK-00137`/`00138`/`00140`.
...
crypto-correctness risk in this feature, comparable to how `TASK-00099` was
flagged as the vault feature's highest-risk task. Expect real SEC.07 review
time here, not a rubber-stamp, per the standing practice `TASK-00089`'s
