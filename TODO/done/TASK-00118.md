...
1. **ARCHITECTURE.md is stale.** Its `Last updated` header reads
   ...
   not started," even though TASK-00094 through TASK-00097 plus TASK-00106 and
   TASK-00109 through TASK-00113 have fully implemented, hardened, and closed
...
- Phase 4 (Sharing): was still wrong even after the 2026-07-29 correction —
  the fabrication was fixed, but by now the real implementation (`TASK-00094`–
  `TASK-00097`, plus `TASK-00106`/`TASK-00109`/`TASK-00111`–`TASK-00113` for
  shared-folder sync) had finished and the row was never updated to say so.
  Marked **complete**.
- Found two entire phases with no table row: Vault/E2EE (`TASK-00089` design,
  `TASK-00098`–`TASK-00101` implementation) and Packaging/deployment/e2e-tests
  (`TASK-00062`–`TASK-00069`) — both referenced only in this doc's own "Last
...
- Phase 8 (Hardening) row updated to reflect its actual, much larger span
  (`TASK-00054`–`TASK-00117`, `TASK-00119`, `TASK-00121`–`TASK-00125`) and that
  `TASK-00120` is the one thing still open.
- While auditing, also found the Approved External Dependencies table and
  ...
  vendored submodules — stale since `TASK-00119`/`TASK-00125` removed both.
  Corrected.
...
`TASK-00119`/`TASK-00125`. The `vw_cache` flag is handled correctly — corrected
