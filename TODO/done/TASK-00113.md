...
Surfaced by CQR.08's review of TASK-00112. In `exec_action()`'s `ACT_UPLOAD`
...
/ `VW_IPC_FILE_MKDIR_REQ` exists (TASK-00104) but is only reachable via an
...
TASK-00112 made this at least *visible* (it's now counted toward the
...
CLI.02 [2026-08-03]: Filed from CQR.08's TASK-00112 review finding. Not
folded into TASK-00112 itself — TASK-00112's scope was narrowly "surface
...
checks), not a status-reporting change. TASK-00112 was updated to at least
...
`run_integration.py` IT-7 sub-test (just fixed in TASK-00110/00112) parses
...
pre-existing TASK-00112 fallback branch in `exec_action()` (`parent_dir_id
...
counting) and rewrote its stale comment (it still referenced TASK-00113 as
...
"no ceiling on total mkdir attempts per cycle, unlike TASK-00111's BFS cap"
...
disproportionate to file, since (unlike TASK-00111's BFS walk, which is
