...
created:     2026-08-11
priority:    high
depends_on:  [TASK-00145]
blocks:      [TASK-00147, TASK-00148, TASK-00149, TASK-00150]
review_by:   [CQR.08]
tags:        [build]
---

Foundation for `TASK-00147`–`TASK-00150`: wire CPack into the existing CMake
install graph with a `server`/`client` component split, per `TASK-00145`'s
design.
...
1. Add `COMPONENT server` / `COMPONENT client` to every existing
   ...
   (`vapourwault-web-gateway`, `TASK-00128`) is out of scope for this task —
...
3. `cpack --list-cmake-cache` / `cpack -G <X> --config CPackConfig.cmake`
   ...
   rpm-build) `TASK-00147`–`TASK-00150` will need.
...
- From a built tree, `cpack -G TGZ` (a trivial, tool-free generator)
  ...
  `TASK-00147`–`TASK-00150` add per-format specifics.
...
design's initial implementation wave. High priority — `TASK-00147`–`TASK-00150`
all build on this.
...
tooling not available in this session (`TASK-00149`/`TASK-00150`'s job to
verify for real), and WIX-specific variables are `TASK-00147`/`TASK-00148`'s
...
thing `TASK-00153`'s feature-level review should double-check doesn't
regress once `TASK-00147`–`TASK-00150` add their own generator-specific
variables on top.
...
invocation time (`TASK-00147`/`00148`/`00151`), so there's no cross-
