...
- Full pytest integration suite passes locally (or in CI) against the real
  server binary — the remaining blocker after TASK-00073's credential fix.

## Notes

<!-- Agents append notes below with their ID and date. Do not delete prior notes. -->

SRV.01 [2026-07-19]: Filed while verifying TASK-00073's fix end-to-end via the
full (non-cluster) pytest integration suite on a Linux/WSL build. 13/20
tests passed (including all of TASK-00073's directly-affected
...
separate, also pre-existing and unrelated — see TASK-00073's notes for
...
- Ran the full real pytest integration suite (`-m "not cluster"`) against
  ...
  - Full `test_dedup.py`, `test_gc.py`, `test_quota.py`, `test_auth.py`,
    ...
    SRV.01's original note above (also flagged in TASK-00073's notes); it is
...
(26/27 tests pass locally, remaining failure is TASK-00075, unrelated).
