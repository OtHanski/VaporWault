...
priority:    normal
depends_on:  []
blocks:      [TASK-00206, TASK-00207, TASK-00208, TASK-00209, TASK-00210, TASK-00211, TASK-00212, TASK-00213]
review_by:   [CQR.08]
tags:        [design]
---

Grew out of a discussion of `TASK-00169`'s client-side fallback: that
...
5. **Caveat, recorded honestly rather than glossed over**: `replica_lag`
   ...
   (`TASK-00169`'s fallback path never touches the primary). A true
...
|------|----------|------------|-----------|------|
| `TASK-00206` — Protocol: `NOTIFY_PREFS_GET`/`_SET` wire spec (user categories only) | PRT.04 | 205 | CQR.08 | protocol |
| `TASK-00207` — Server: user-category preference storage + triggers (`share_received`, `quota_warning`, `new_login`, `account_security_change`) | SRV.01 | 206 | SEC.07, CQR.08 | security-sensitive, server |
| `TASK-00208` — Server: admin-category config + triggers (`replica_lag`, `acme_renewal_failure`, `disk_capacity`, `lockout_spike`, `crash_recovery`) | SRV.01 | 207 | SEC.07, CQR.08 | security-sensitive, server |
| `TASK-00209` — Client: daemon IPC + `vapourwault-cli notify` for user categories | CLI.02 | 207 | CQR.08 | client |
| `TASK-00210` — GUI: notification-preferences panel | GUI.03 | 209 | CQR.08 | gui |
| `TASK-00211` — Web gateway/frontend: notification-preferences UI | WEB.09 | 207 | SEC.07, CQR.08 | security-sensitive, gateway |
| `TASK-00212` — Docs: admin config keys + user-facing toggle documentation | BLD.05 | 207-211 | CQR.08 | docs |
| `TASK-00213` — Integration tests: every trigger, debounce behavior, default-off regression | QA.06 | 206-212 | CQR.08 | test |
...
ARCH.00 [2026-08-27]: **Milestone closed.** `TASK-00206`–`213` all `done`,
...
`TASK-00204`'s audit) updated from "design complete, implementation in
progress" to "complete" as part of this same closure — per `TASK-00204`'s
...
absorbed into this milestone's own scope: `TASK-00219` (no self-service
2FA enable/disable exists, so `account_security_change` only fires for
password changes today) and `TASK-00222` (no wire path sets a user's email
at all, discovered by `TASK-00213` — `priority: high`, since it also
silently breaks the already-shipped `TASK-00046` password-recovery
