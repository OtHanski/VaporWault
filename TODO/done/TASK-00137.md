...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127, TASK-00132, TASK-00136]
blocks:      [TASK-00138]
review_by:   [SEC.07, CQR.08]
tags:        [web, security-sensitive]
---

Build the login view against `TASK-00132`'s `/api/login`, `/api/login/otp`,
`/api/logout` endpoints, using `TASK-00136`'s scaffold.
...
- Session persistence across page reloads (the gateway session cookie,
  `TASK-00131`, should just work via the browser's normal cookie handling —
  confirm `Secure`/`HttpOnly`/`SameSite` attributes are set gateway-side in
  `TASK-00131`/`TASK-00132`, not left to the frontend to compensate for).
- Logout action.
...
browser — no GUI browser available in this environment. `TASK-00136`'s note
...
`TASK-00138`/`00140` as well (see those tasks' notes) — noted once here,
