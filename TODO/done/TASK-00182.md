...
This task is IPC + CLI only. GUI surfacing is `TASK-00183`, blocked on this.
...
- Daemon handler (`vw_daemon.c`): resolves the account's active session
  ...
  fallback per `TASK-00173`'s existing read-allowlist; `VERSION_RESTORE` is a
...
- Attempting `version restore` while the account is on fallback
  (`TASK-00173`) fails with a clear error, not a hang or a wire-level
...
- Verified for real, not just "it compiles": built clean on both
  ...
  `VW_WERROR=ON` — see the TASK-00215 note below, unrelated to this task's
...
- **Discovery, filed separately rather than fixed here (routing rule 4)**:
  ...
  variant (`TASK-00095`/`00106`'s precedent). A grantee — VIEW or EDIT, it
  doesn't matter which — cannot list or restore versions of a file
  shared with them at all today. Filed as `TASK-00214` (PRT.04) rather
...
- **Second discovery, also filed separately**: a single local file
  ...
  touches. Filed as `TASK-00215` (CLI.02).
