...
created:     2026-07-29
priority:    high
depends_on:  [TASK-00094, TASK-00095]
blocks:      []
review_by:   [SEC.07, CQR.08]
tags:        [testing, security-sensitive]
---

Write regression tests for the sharing feature (`TASK-00088`,
`docs/PROTOCOL.md` §7.5/§7.10). This is a security-sensitive feature per
routing rule 1, and every SEC.07 finding resolved against `TASK-00094`/
`TASK-00095` needs a corresponding regression test per the standing QA.06
responsibility in `CLAUDE.md`.

Scope (minimum, expand based on SEC.07 findings from TASK-00094/00095):
...
- Public link: create, redeem anonymously (no account), use within
  ...
  `TASK-00088`'s design notes as easy to get wrong.
...
- Sign-off note added to `TASK-00094`/`TASK-00095` before ARCH.00 closes the
  sharing milestone.
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00088`.
...
Reviewed `tests/integration/test_sharing.py` (the existing TASK-00094
...
there is still no wire message that creates one (`TASK-00104`, filed
2026-07-30, still `todo`). `test_sharing.py`'s own module docstring already
disclosed this gap when `TASK-00094` was written; the cycle-detection logic
...
Revisit once `TASK-00104` lands — do not consider this task's FILE_MOVE
...
findings. Closing as done. `TASK-00088`'s sharing milestone
(`TASK-00094`/`TASK-00095`/`TASK-00097`) is now fully closed; `TASK-00096` (GUI)
remains open, and `TASK-00104`/`TASK-00106` remain as disclosed, filed
follow-up gaps (no wire `mkdir`; sync-engine tree integration for shared
folders).

QA.06 [2026-07-31]: `TASK-00104` closed shortly after this task, unblocking
