...
priority:    high
depends_on:  []
blocks:      [TASK-00170, TASK-00171, TASK-00172, TASK-00173, TASK-00176]
review_by:   [CQR.08]
tags:        [design]
---
...
it ever syncs the content. Recorded as its own decision/task (`TASK-00171`)
...
|------|----------|------------|-----------|------|
| `TASK-00170` — Protocol: publish `CLUSTER_RECORD_*`/`CLUSTER_CHUNK_*` wire spec in `docs/PROTOCOL.md` | PRT.04 | 169 | CQR.08 | protocol |
| `TASK-00171` — Server: GC replica-safety gating (`vw_gc.c` chunk deletion gated on `vw_cluster_min_sync_watermark()`) | SRV.01 | 169 | SEC.07, CQR.08 | security-sensitive, server |
| `TASK-00172` — Server: replica hot-standby data replication (record fetch + chunk sync, extends `vw_cluster.c`'s replica pull loop) | SRV.01 | 170, 171 | SEC.07, CQR.08 | security-sensitive, server |
| `TASK-00173` — Daemon: per-account fallback config + read-only failover connection logic + offline-queue integration | CLI.02 | 172 | SEC.07, CQR.08 | security-sensitive, client |
| `TASK-00174` — `vapourwault-cli`: `--fallback-host`/`--fallback-port`/`--fallback-ca-cert` on `account add`, shown in `account list` | CLI.02 | 173 | CQR.08 | client |
| `TASK-00175` — GUI: fallback fields in the "Add account" dialog, a visible "read-only (fallback)" indicator distinct from plain offline | GUI.03 | 173 | CQR.08 | gui |
| `TASK-00176` — Gateway: `--fallback-server-host`/`--fallback-server-port`/`--fallback-ca-cert` + read-only failover connection logic | WEB.09 | 172 | SEC.07, CQR.08 | security-sensitive, gateway |
| `TASK-00177` — Docs: `docs/DEPLOYMENT.md` end-to-end primary+replica+fallback setup walkthrough | BLD.05 | 172, 173, 176 | CQR.08 | docs |
| `TASK-00178` — Integration tests: replica hot-standby correctness, GC-safety regression, daemon/gateway automatic fallback (read-only success + write-queue-and-flush), replica-lag chunk-safety | QA.06 | 170–177 | CQR.08 | test |
...
breakdown above created directly in `TODO/todo/`. Proceeding to `TASK-00170`
next (protocol spec must land before `TASK-00172`'s implementation, per
