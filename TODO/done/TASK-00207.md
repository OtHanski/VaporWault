...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00206]
blocks:      [TASK-00208, TASK-00209, TASK-00211, TASK-00212, TASK-00213]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, server]
---

Implements `TASK-00206`'s wire spec plus the four user-category triggers
from `TASK-00205`'s design. Introduces a small shared dispatch helper
...
state for threshold-style categories) that `TASK-00208`'s admin categories
...
- **account_security_change**: fires from `handle_recover_confirm`
  ...
  change. **Correction to `TASK-00205`'s design assumption, found by
  ...
  0). Filed `TASK-00219` (design gap, ARCH.00) rather than either
...
- **Known gap, filed rather than fixed here**: `notify_prefs.db` is not
  part of `TASK-00172`'s replica hot-standby file-tag replication list — a
  ...
  wire-documented table is a protocol change; filed as `TASK-00220`
...
- `NOTIFY_PREFS_SET`'s reserved-bit rejection (`VW_ERR_INVALID_ARG` for
  ...
  before the store write. Noted for `TASK-00213`: no wire-level test yet
  ...
  validation) — not blocking, since `TASK-00213` is explicitly scoped to
  this level of coverage, but flagging so it isn't forgotten.
- The two follow-ups filed (`TASK-00219`: no 2FA-toggle mechanism exists to
  hook; `TASK-00220`: notify_prefs.db not replicated to hot-standby
