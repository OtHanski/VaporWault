...
created:     2026-07-13
priority:    normal
depends_on:  [TASK-00051, TASK-00050]
blocks:      []
review_by:   [CQR.08]
...
dashboard scaffolding (TASK-00051) and the `CLUSTER_STATUS` handler on the
server (TASK-00050).
...
Use the same non-blocking async recv model from TASK-00052.
...
ARCH.00 [2026-07-13]: TASK-00050 must be done first so CLUSTER_STATUS_RESP
actually returns node data. This task can be started on the UI scaffold side
(render the view with mocked data) before TASK-00050 is complete, but the wired
live version requires TASK-00050. The lag colouring thresholds (0 / 1-999 / 1000+)
...
- `src/gui/server/views/vw_view_dashboard.cpp` — `on_connected` now calls
  ...
  — TASK-00053)")` with `cluster_view_.render(app)`.
...
handler in `vw_file_handlers.c` was updated in TASK-00050 to match.
...
- CMakeLists: `vw_view_cluster.cpp` added; server include path already present
  from TASK-00052 for proto.h helpers. ✓

Advisory: `do_fetch` is synchronous (blocks render thread during send+recv);
same advisory as TASK-00052 — acceptable for Phase 7, async queue in Phase 8.
...
ARCH.00 [2026-07-13]: CQR.08 signed off. No blocking findings. TASK-00053 done.
Phase 7 milestone complete: all TASK-00047 through TASK-00053 are done.
