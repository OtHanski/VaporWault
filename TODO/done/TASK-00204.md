...
- The Implementation Phases table's Phase 11 (web gateway) row still
  ...
  `TASK-00128`-`144` are all `done` in `TODO/done/` and the gateway/
  frontend shipped in commit `a5a54fd`.
- The Phase 12 (multi-account) row still reads "design published,
  implementation not started," but `TASK-00161`-`168` are all `done` and
...
- `TASK-00169`-`181` (replica hot-standby, client fallback, chunk-refcount
  ...
  consistent with how `TASK-00169`'s own Decision Log entries already
...
- Re-audit every phase-status claim in the Implementation Phases table
  ...
  field (all of them, not a sample — repeat `TASK-00118`'s method, since
...
- Add/update phase rows for anything shipped since the last audit
  ...
  new design tasks filed this session (`TASK-00182`, `185`, `192`, `196` and
...
- Corrected Phase 11 (Web gateway) and Phase 12 (Multi-account) to
  **complete** — both were still "design published, implementation not
  started" despite every task in each wave (`TASK-00128`-`144`,
  `TASK-00161`-`168`) being `done`.
- Added Phases 13-19 for entire feature waves that had shipped with no
  table row at all: installer packages (`TASK-00145`-`154`, one item —
  `TASK-00152` — still open, called out honestly rather than marked
  complete), replica hot-standby + client fallback (`TASK-00169`-`181`),
  client version history (`TASK-00182`-`184`, plus the still-open shared-file
  gap `TASK-00214`), public link password protection (`TASK-00185`-`191`),
  selective sync (`TASK-00192`-`195`), filename search (`TASK-00196`-`202`),
  and version-string embedding (`TASK-00203`).
- Added Phase 20 for the opt-in email alert system (`TASK-00205`-`213`),
...
- Extended Phase 8's (Hardening) description to name the bug-fix tasks
  ...
  (`TASK-00155`-`157`, `159`, `217`, `218`) rather than leaving them
  undocumented.
