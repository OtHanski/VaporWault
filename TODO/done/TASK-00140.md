...
created:     2026-08-10
priority:    normal
depends_on:  [TASK-00127, TASK-00134, TASK-00138]
blocks:      []
...
`TASK-00133`'s version endpoints) and sharing/public-link management (grant/
revoke/list shares, create/revoke/list public links, against `TASK-00134`'s
endpoints).
...
corresponding thin wrappers over `TASK-00133`'s version endpoints and
`TASK-00134`'s sharing endpoints.
...
`TASK-00134`, has no frontend caller). `TASK-00140`'s own acceptance criteria
...
under `TASK-00137`'s note — not re-fixed here for the same reason (belongs
