...
the running daemon via IPC (TASK-00027) and issues commands.

## Acceptance criteria

### 1. Entry point integration

The `vapourwault-client` binary (from TASK-00030) accepts subcommands. If the
...
The TASK-00031 spec §1 describes a single merged binary, but the existing CMakeLists.txt
...
Sign-off: CQR.08 approves TASK-00031 for done.
...
structure takes precedence. TASK-00031 done.
