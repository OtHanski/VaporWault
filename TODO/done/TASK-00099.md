...
created:     2026-07-29
priority:    normal
depends_on:  [TASK-00089, TASK-00098]
blocks:      [TASK-00100, TASK-00101]
review_by:   [SEC.07, CQR.08]
tags:        [client, crypto, security-sensitive]
---

Implement the client side of the E2EE design published in
`docs/PROTOCOL.md` §7.11 (`TASK-00089`): all actual cryptography lives here,
never server-side.
...
- Passphrase handling: must reuse secure-memory handling already used for
  ...
  to the login path in `TASK-00087` (zeroing partial secrets on early-return
  paths).
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00089`. This is the
...
1. **Wire gap closed** (flagged in `TASK-00098`'s own notes, §7.11.4):
...
3. **`vw_client_core.h`/`.c`** extended with wire-only primitives (no key
   ...
   (a genuine gap: `FILE_MKDIR` has existed on the wire since `TASK-00104`
...
4. **New module `src/client/vw_vault.h`/`.c`** (distinct from
   `src/server/vw_vault.h`/`.c`, TASK-00098's server-side storage module):
...
- No advisory or blocking findings beyond the two already resolved during
  design (`TASK-00089`'s nonce-scheme revision and the Argon2id floor,
...
acceptance criteria satisfied. Closing `TASK-00099` as `done`. `TASK-00100`
(GUI vault UI) and `TASK-00101` (E2EE regression tests) are now unblocked.

QA.06 [2026-07-31]: `TASK-00101`'s regression suite exercises every
...
(a fresh reviewer, not the original implementer) of `TASK-00099`–`TASK-00101`
...
1. **(High, functional) `vw_vault_unlock()` never populated
   ...
   as an invalid absolute path. Neither this task's nor `TASK-00101`'s test
