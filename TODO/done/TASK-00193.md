...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00192]
blocks:      [TASK-00194, TASK-00195]
review_by:   [CQR.08]
tags:        [client]
---

Implements `TASK-00192`'s design.
...
- `vw_sync.c`: `srv_collect` and its `remote_dir_id`-rooted variant
  (`TASK-00106`) skip any entry whose relative path matches an exclude
  rule, before it ever becomes an `action_t`. Excluding a path that's
  already synced locally leaves the local copy alone (per `TASK-00192`
...
- Vault-rooted sync folders reject rule assignment with a clear error
  (per `TASK-00192` decision 6).

## Notes

<!-- Agents append notes below with their ID and date. Do not delete prior notes. -->

CLI.02 [2026-08-26]: **Two corrections to `TASK-00192`'s design, found by
