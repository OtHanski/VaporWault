...
created:     2026-08-11
priority:    high
depends_on:  [TASK-00145, TASK-00146]
blocks:      []
review_by:   [SEC.07, CQR.08]
tags:        [build, windows, security-sensitive]
---

Build a proper client MSI via CPack's WIX generator. Per `TASK-00145`'s
...
1. `CPACK_GENERATOR "WIX"` invocation for the `client` component, with a
   distinct `CPACK_WIX_UPGRADE_GUID` from the server package's (`TASK-00147`)
...
3. A deferred custom action invoking the scheduled-task-registration logic
   ...
   - **Argument handling**: any path/property value passed from WiX into
     ...
     this with the same rigor `TASK-00081` applied to `release.yml`'s
     ref-name handling (SEC.07 flagged this class of risk generally during
     `TASK-00145`'s design review). Prefer passing values via a temp
...
- Built and verified in this session — NOT installed on a real machine
  ...
  install/uninstall verification is `TASK-00152`'s job.
...
exactly the kind of pattern `TASK-00081` already had to fix once elsewhere in
this project's CI; do not repeat that mistake here.

BLD.05 [2026-08-11]: While implementing `TASK-00147`, generated a second
...
Also confirmed via `TASK-00147`'s testing that CPack's WIX generator needs
...
this task adds) — see `TASK-00147`'s implementation note for the exact
...
invention. On SEC.07's command-injection concern from `TASK-00145`'s design
...
to cross here, unlike `TASK-00147`'s elevated server install. Flagging for
`TASK-00153` to confirm this reasoning, not just accept it.
...
machine — flagged for whoever runs `TASK-00152` next, ideally on a personal/
...
direct reading, matching `TASK-00153`'s independent elevation-scope
...
QA.06 [2026-09-07]: **Correction, found during `TASK-00152`'s real-install
verification — this task's central claim above did not hold.**
`CPACK_WIX_INSTALL_SCOPE=perUser` is not a real CMake/CPack variable (checked
directly: `cmake --help-variable CPACK_WIX_INSTALL_SCOPE` says "is not a
defined variable"; it's absent from CMake 3.28's full documented
`CPACK_WIX_*` list). Setting it via `-D` silently defines an unused variable
and does nothing. Confirmed against the actual built MSI's `Directory` table
via COM automation (same technique this task used for the `CustomAction`
table): `TARGETDIR`'s only child is `ProgramFiles64Folder`, not
`LocalAppDataFolder`. A real non-elevated `msiexec /i` (via a de-elevated
`RunLevel Limited` scheduled task, to test the actual target scenario) fails
outright with `Error 1303: insufficient privileges to access ... Program
Files...` — the exact failure the per-user design exists to prevent. An
elevated install "succeeds" but installs to Program Files under an elevated
context, and the `RegisterClientTask`/`UnregisterClientTask` custom actions
then both fail silently (masked by `Return="ignore"`). Filed as `TASK-00247`
(critical, security-sensitive) rather than fixed here — this affects a
SEC.07-mandated security design constraint and needs a real fix plus
re-review, not a quick patch. Not reopening this task's `done` status (the
fix belongs to the new task), but flagging clearly: this task's own
"verified" claim about `directories.wxs` was not actually exercised against a
real install, and should not be trusted at face value in future similar
claims — verify against the built artifact's actual tables, not just
