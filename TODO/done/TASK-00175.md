...
Surfaces `TASK-00173`'s new per-account fallback config in the GUI.

## Work

- "Add account" dialog (`vw_view_login.cpp`, extended in `TASK-00163`):
  optional fallback host/port/CA-cert fields, same
  all-three-or-none validation as `TASK-00174`'s CLI.
- A clear, distinct visual state for "connected to fallback (read-only)"
  ...
  account switcher (`TASK-00163`) is the natural place, since it already
...
- Write-attempting UI actions (upload button, mkdir, delete, share
  ...
  already queues rather than rejecting outright (`TASK-00173`), so the GUI
...
- "Add account" dialog (`vw_view_login.cpp`): a checkbox reveals
  ...
  independently optional, same interpretation TASK-00174's CLI used) before
...
- **Found and fixed the same `ACCOUNT_LIST_RESP`/`STATUS_RESP` decode gaps
  TASK-00173/00174 had already found in the CLI, this time in
...
- **Correction to this task's own premise, worth flagging explicitly**:
  ...
  watched folder while on fallback — `TASK-00173`'s offline-queue path,
  ...
  `TASK-00173` explicitly rejects those outright (`VW_ERR_READ_ONLY_FALLBACK`)
