...
created:     2026-08-04
priority:    normal
depends_on:  [TASK-00121, TASK-00122]
blocks:      [TASK-00124]
...
("Oplog entry format (v17)"), resolving `TASK-00121` (timestamp) and
`TASK-00122` (FILE_WRITE disambiguation) together as one migration, per that
spec's suggestion.
...
- `src/gui/server/views/vw_view_audit.cpp` (`OPLOG_ENTRY_HDR = 17u`,
  `vw_view_audit.cpp:26`): also update to the new size — this is TASK-00124's
  responsibility, but flagging here since it's the same constant-divergence
  bug class as the `vw_cluster.c` finding above and TASK-00124 depends on
...
- Unit tests (mirroring the existing oplog test coverage from
  `TASK-00114`/`TASK-00115`) for: CRC still validates with the new field
...
- Do not start until `TASK-00121` and `TASK-00122` are `done` (spec is
...
`docs/PROTOCOL.md` §7.7 (`TASK-00121`/`TASK-00122`). Blocks `TASK-00124` (GUI.03)
...
- `vw_cluster.c`: fixed the replica-side batch-splitting loop's two `17u`
  literals as SEC.07 flagged on `TASK-00121`/`TASK-00122`. **Also found and
