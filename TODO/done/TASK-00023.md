...
This module sits above `vw_store` (TASK-00022) and `vw_fs`. It does not touch the wire;
TASK-00024 calls into it from the server request handlers.
...
- Sets bit `i` of `out_bitmask` if the server has chunk `i` (big-endian bit order within
  each byte, as per TASK-00021 §2). Caller must provide `ceil(count/8)` bytes.
- `count` max is 1024 (enforced by TASK-00021 protocol spec). Return
...
4. **Ref-count ordering invariant** (TASK-00023 §1): ref_count is appended to disk
...
**Deviations from TASK-00023 spec:**
...
(TASK-00024) must NOT re-expose this data without considering that on-disk corruption is
...
Sign-off granted. TASK-00024 is now unblocked.

---

ARCH.00 [2026-07-11]: TASK-00023 closed. All sign-offs received; no blocking findings.
Chunk store implementation is complete. TASK-00024 (server request handlers) is now
fully unblocked (TASK-00021 done, TASK-00022 done, TASK-00023 done).
