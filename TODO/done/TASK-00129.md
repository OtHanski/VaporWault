...
Per `TASK-00127`'s design decision, the gateway hand-rolls its own minimal
...
- Support `GET`, `POST`, `PUT`, `DELETE` — the methods the endpoint set in
  `TASK-00132`–`TASK-00135` actually needs.
...
- **Default bind address is `127.0.0.1` only** (`ARCHITECTURE.md`'s Gateway
  listener bind address decision, added during `TASK-00127`'s SEC.07 review).
...
- Parses every request shape the endpoint tasks (`TASK-00132`–`TASK-00135`)
...
- Default bind address is left as a caller-supplied parameter (not
  hardcoded in this module) — `TASK-00131`'s gateway startup code is
...
(`TASK-00143`) covers truncated headers and oversized `Content-Length`
