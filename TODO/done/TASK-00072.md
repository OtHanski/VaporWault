...
still fully red after TASK-00070 landed (commit c384861) — but now with a
...
- This exact test failed with
  ...
  handshake failure (_ssl.c:1000)` in the **original pre-TASK-00070 CI run
  too** — proving this is a separate, pre-existing bug in the real TLS
  server, not something TASK-00070's fix caused.
- It was previously masked because every other integration test called
  `make_user()` first, which hit TASK-00070's struct.error bug and errored
...
- Relevant server-side TLS config, `src/core/vw_net.c`:
  ...
  - `src/core/vw_crypto.c:94-96` has a suggestive comment: "PSA Crypto
    ...
    TLS handshake hangs or crashes on some platforms." TASK-00070 made
...
`review_by`. `depends_on: [TASK-00070]` recorded for lineage (TASK-00070 is
...
TASK-00070's own acceptance criteria (full pytest + TAP suite passing
...
TASK-00070's PSA-init fix (confirmed by testing: the bug reproduces
...
be committed and pushed next alongside TASK-00070; CI re-run provides final
