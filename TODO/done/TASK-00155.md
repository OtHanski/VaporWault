...
Discovered while implementing the web gateway's file endpoints (`TASK-00133`)
...
Via the web gateway (`TASK-00133`) against a real server:
...
- `TASK-00133`'s gateway mitigation (closing a session's connection on an
...
WEB.09 [2026-08-11]: Filed while implementing `TASK-00133` (gateway file
...
added gateway-side instead (see `TASK-00133`'s implementation note).
...
- Full existing suite reruns clean: 16/16 `ctest` (MSVC, including the
  ...
  the exact `TASK-00133`-reported real-world path (a rejected file
  operation through the gateway) with the fix in place, not just the new
  isolated unit test.

**On `TASK-00133`'s gateway-side mitigation**: per this task's own
