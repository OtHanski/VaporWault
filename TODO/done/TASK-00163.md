...
- Views (`vw_view_browser.cpp` and friends) do **not** need per-account
  ...
  accounts is entirely the daemon's concern (`TASK-00161`), decoupled from
...
- "Add account" flow (Settings view or a new dialog): collects
  ...
  - `VwGuiIpc::login()`'s existing `ACCOUNT_ADD_REQ` payload buffer (`uint8_t
    buf[900]`, `TASK-00161`) has unchecked `vw_ipc_write_str()` calls — safe
    ...
    `TASK-00162`'s CQR review applied to the CLI's equivalent function is
...
- With two accounts configured (via `TASK-00161`'s daemon), switching in the
...
- `login()` was replaced with `account_add()`/`account_list()`/
  `account_remove()`, mirroring `TASK-00162`'s CLI `account` subcommands
  ...
  flagged the exact bug `TASK-00162`'s CQR pass found in the CLI's
...
- Regression found and fixed while doing this (see CQR.08 note below):
  ...
  ever attempting a build, by diffing against `git show <TASK-00108
...
pre-flagged from `TASK-00162`) was correct from first write, confirmed by
