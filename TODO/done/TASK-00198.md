...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00197]
blocks:      [TASK-00199, TASK-00201, TASK-00202]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, server]
---

Implements `TASK-00197`'s wire spec.

**Correction (2026-08-26)**: this Work section still described the
pre-correction cursor-pagination design struck in `TASK-00196`/`00197`. The
wire spec `TASK-00197` actually published has no pagination and no
...
- This is a new parser entry point (query string parsing) — in scope for
  QA.06's standing fuzz-testing policy (`TASK-00202`).
...
- New integration test `tests/integration/test_search.c` +
  ...
  `vw_client_search()` wrapper exists yet — that's `TASK-00199`). 29
  ...
  - Also fixed a pre-existing binary-discovery gap while wiring this
    ...
    `_find_client_bin` helper (`TASK-00161`+) does. Added it to
    ...
    project-wide. Filing `TASK-00217` for BLD.05/QA.06 to reconcile all
...
- Full regression check: `ctest --test-dir build-gw-e2e` (all 19
  ...
  after this change (see `TASK-00217` for the one adjacent, unrelated gap
