...
**Settled scope note (2026-08-13):** unlike the local daemon (`TASK-00161`),
...
- New `/api/accounts` (no `require_session()` — an empty result is a
  ...
  cookie already present — TASK-00166's job to actually wire that up).
...
- Test fix: `test_invalid_cookie_is_rejected` (TASK-00144 regression test)
...
**QA.06 [2026-08-13] — TASK-00168 sign-off:** `TASK-00168`'s "gateway
...
`security-sensitive`; `TASK-00165` is the security-sensitive one in this
