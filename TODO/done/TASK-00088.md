...
claimed sharing was "complete", citing TASK-00034/00035 (actually CI setup and
...
- **Public-link mechanism**: reuses the existing `INVITE_REDEEM` precedent —
  ...
  growing the struct — same "reuse reserved bytes" pattern as `TASK-00090`'s
  `deleted_at`.
...
Follow-up implementation tasks filed with `depends_on: [TASK-00088]`:
`TASK-00094` (SRV.01 — server storage/permission/protocol implementation),
`TASK-00095` (CLI.02 — client library + sync engine), `TASK-00096` (GUI.03 —
share/link UI, depends on TASK-00095 per the existing GUI.03 constraint),
`TASK-00097` (QA.06 — integration tests, depends on TASK-00094/00095).
...
(§7.5/§7.10) before `TASK-00094` starts implementation.
...
1. **FILE_MOVE crosses the sharing boundary undefined.** Nothing in §7.5
   ...
   *destination* `parent_dir_id`, nor does it forbid moving a folder under
   ...
   walk-up check for that subtree. `TASK-00094` must specify: authorize both
...
- Same-request TOCTOU: the design says permission must be re-derived "on
  ...
  a locking note in `TASK-00094`, not blocking the design sign-off.
...
1. **`permission` field collides with the existing, already-used
   ...
   `vw_perm_t.EDIT=2`. Neither `TASK-00094` nor `TASK-00096` (which needs the
...
2. **`permission_needed_for_this_op` is used in the permission-check rule
   ...
   `owner_id` does the new file get?). Without a table, SRV.01 (`TASK-00094`)
   and QA.06 (`TASK-00097`) would each have had to invent this independently
   and likely disagree.
3. **`blocks`/`depends_on` graph asymmetry**: `TASK-00094.blocks` listed
   `TASK-00096`, but `TASK-00096.depends_on` only listed `TASK-00095` (correctly,
   since GUI consumes the client library only — but that left `TASK-00094`'s
   `blocks` entry overstated/orphaned). Same pattern between `TASK-00096.blocks`
   → `TASK-00097` and `TASK-00097.depends_on`.
4. **`TASK-00099`'s `depends_on` omitted `TASK-00098`** despite its own
   acceptance criteria requiring a live round-trip against the real server
   (which needs `TASK-00098`'s handlers) — the sharing side's equivalent task
   (`TASK-00095`) got this right by depending on `TASK-00094` for the same
...
- `LINK_ACCESS_ACK`'s scoped session gives no way for the redeeming client
  ...
  before `TASK-00094`/`TASK-00095` implementation diverges on it.
- `TASK-00095`'s note ("client-side work can start in parallel") reads oddly
  next to its own hard `depends_on: [TASK-00088, TASK-00094]` — a one-line
...
- CQR.08 #3/#4: fixed the `blocks`/`depends_on` graph (`TASK-00094.blocks`
  and `TASK-00096.blocks` overstated entries removed; `TASK-00099.depends_on`
  now includes `TASK-00098`).
...
- SEC.07 advisory (LINK_ACCESS rate-limit table sizing): left as an
  implementation-time decision for `TASK-00094` — noted there, not blocking.

All follow-up subtasks (`TASK-00094`–`TASK-00097`) updated to reference these
...
closing this task, matching the practice established for `TASK-00085` earlier
...
§7.5/§7.10 and the `TASK-00094`–`TASK-00097` files — quoted evidence for each,
...
Moving status to `done`. `TASK-00094` (SRV.01), `TASK-00095` (CLI.02),
`TASK-00096` (GUI.03), `TASK-00097` (QA.06) are cleared to start per their
`depends_on` edges.
