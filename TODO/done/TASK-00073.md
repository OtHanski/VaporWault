...
created:     2026-07-19
priority:    critical
depends_on:  [TASK-00070, TASK-00072]
blocks:      []
review_by:   [SEC.07, CQR.08]
tags:        [bug, ci, auth, crypto, security-sensitive]
---

With TASK-00070 and TASK-00072 both landed and confirmed in CI run 29683468506
...
2. **Login** — `handle_auth_request` (`src/server/vw_server_core.c:105-193`)
   ...
   Note (Phase 1 issue TASK-00009): For Phase 0 the wire token is
...
pre-hash step. This has nothing to do with TASK-00070/00072; it is a separate,
...
TASK-00070's reasoning: the correct derivation convention already exists and
...
both SEC.07 and CQR.08 in `review_by`. `depends_on: [TASK-00070, TASK-00072]`
...
- Ran the fuller non-cluster suite (20 tests, excluding `-m cluster`): 13
  ...
  **separate, pre-existing, unrelated bugs** independent of credential
  derivation — filed as **TASK-00074** (admin socket handles only one
...
the remaining failures are independent pre-existing bugs (TASK-00074 and one
...
- **Zeroing**: `pw_token` is zeroed on both exits — the early return on
  ...
  path found.
- **Advisory (non-blocking), consistent with today's TASK-00072 precedent**:
  both new comment blocks bake in dated task-attribution narrative — `/*
  SRV.01 [2026-07-19] TASK-00073: ... */` in `vw_admin.c` (10 lines) and
  `TASK-00073 [SRV.01, 2026-07-19]: fixed a mismatch where...` in `vw_admin.h`
  ...
  existing one-line `SRV.01 [2026-07-12]: TASK-00040 implementation.` header
...
2. **Admin socket safety claim verified independently, not taken on faith.**
   ...
   mode bit — but that's pre-existing scope from TASK-00040, not touched or
...
advisory, same as TASK-00072 — kept the essential why, dropped the
dated task-attribution narrative. Confirming resolution and closing —
status: done. Filed `TASK-00077` for CQR.08's other advisory (`vw_admin.c`
...
task are tracked separately: TASK-00074 (admin socket connection reuse),
TASK-00075 (brute-force lockout), TASK-00076 (quota accounting) — not
