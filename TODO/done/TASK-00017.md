...
issue in vw_smtp. All three must be resolved before TASK-00009 (vw_auth) is picked up,
...
SEC.07 must sign off on this fix before TASK-00009 (vw_auth_provider) is implemented.
...
must sign off before TASK-00009 is started. Items 2 and 3 are also required before
...
therefore listed as blocking for TASK-00009.

SRV.01 owns vw_smtp (implemented in TASK-00010); all changes go into
...
Requesting SEC.07 review of the from_addr fix before TASK-00009 picks up.
...
CQR.08 sign-off: TASK-00017 may proceed to `done` once SEC.07 confirms its advisory items
are addressed (or waived by ARCH.00 with documented rationale).

ARCH.00 [2026-07-06]: Closing TASK-00017. All three acceptance criteria verified complete by
