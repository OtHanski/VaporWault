...
During TASK-00007 closure review, a platform-specific gap in the BLOCKING-E fix was
...
consequence when it does occur is the same as the pre-TASK-00007-fix state: the next
...
of TASK-00007 BLOCKING-C/D/E fixes. TASK-00007 is closed; this is a follow-up advisory fix.
...
for TASK-00008 — vw_store can start while this task is in progress.
...
2. `fd_write` failure path (~line 724): comment expanded to reference the GENERIC_READ
   fix and note that TASK-00019 resolved the fd_size silently returning -1 on Windows.
...
Advisory CQR.08-A-1 noted for style only — no action required. TASK-00019 is done.
