...
created:     2026-08-04
priority:    normal
depends_on:  [TASK-00123]
blocks:      []
review_by:   [CQR.08]
tags:        [gui, audit]
---

Once `TASK-00123` (SRV.01) lands the server-side oplog v17 format from
`docs/PROTOCOL.md` §7.7, update `src/gui/server/views/vw_view_audit.cpp` /
`.h` (from `TASK-00116`) to consume the new fields. This restores the
date/time filter deferred in `TASK-00121` and the FILE_WRITE user-id
attribution removed as a workaround in `TASK-00122`.
...
- Add a date/time-range filter UI control (the acceptance criterion
  deferred from `TASK-00116`/filed as `TASK-00121`) using the new timestamp,
...
- Replace the single `VW_OPLOG_FILE_WRITE` branch in `parse_audit_resp`
  (`vw_view_audit.cpp:207-214`, the `ref_id=` workaround from `TASK-00122`)
  ...
  - `FILE_CREATE`: restore `owner_id=` labeling and `has_subject = true`
    ...
    this is the capability TASK-00122 flagged as lost.
...
PRT.04 [2026-08-04]: Filed alongside `TASK-00123` once the v17 oplog format
was published in `docs/PROTOCOL.md` §7.7, resolving `TASK-00121`/`TASK-00122`'s
GUI-side follow-up. Depends on `TASK-00123` since the new wire bytes must
...
- Field shipped as `ts_unix_secs`, not `ts_unix_ms` as this task originally
  said — see `TASK-00123`'s SRV.01 note / `docs/PROTOCOL.md` §7.7's
...
- `OPLOG_ENTRY_HDR` (the private `17u` constant) removed entirely rather
  ...
  hardcoded copies SEC.07 flagged during the `TASK-00121`/`TASK-00122` review;
...
- **Verified**: full local build (`build-msvc-105`, MSVC, `/W4 /WX`)
  ...
  on in `TASK-00123` (same offsets, same constant).

CQR.08 [2026-08-04]: Reviewed `TASK-00123`+`TASK-00124` together given how
...
verify paths — consistent with the `TASK-00121` finding this was meant to
...
either task. Marking `TASK-00123` and `TASK-00124` `done`.
