...
Discovered while writing `tests/integration/test_sharing.py` for `TASK-00094`:
...
all, before or after `TASK-00094`. This is why `TASK-00094`'s own integration
...
structure, GUI "New Folder" actions, `TASK-00096`'s sharing UI needing to let
...
integration-testing TASK-00094, not something TASK-00094 itself should fix
...
separately" reasoning as `TASK-00105`'s note on that distinction):**
`handle_file_commit`'s "creating a new file under a shared folder" branch
(added in `TASK-00094`, for a `file_id` that names a directory) has been
...
- `tests/integration/test_sharing.py`: `test_folder_share_view_grant_
  ...
  `TASK-00097`'s own disclosed gap while here: `test_file_move_rejects_
  ...
  into/around, which is exactly what this task unblocks. See `TASK-00097`'s
...
Closing as done. `TASK-00096` (GUI sharing) can now offer a real "share a
folder you created" flow, and `TASK-00106` (sync-engine folder awareness)
