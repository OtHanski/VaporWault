...
created:     2026-07-29
priority:    normal
depends_on:  [TASK-00098, TASK-00099]
blocks:      []
review_by:   [CQR.08]
tags:        [testing, crypto, security-sensitive]
---

Write regression tests for the E2EE feature (`TASK-00089`,
`docs/PROTOCOL.md` §7.11). Security-sensitive per routing rule 1; every
SEC.07 finding resolved against `TASK-00098`/`TASK-00099` needs a corresponding
regression test.

Scope (minimum, expand based on SEC.07 findings from TASK-00098/00099):
...
- Sign-off note added to `TASK-00098`/`TASK-00099` before ARCH.00 closes the
  E2EE milestone.
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00089`.
...
running server, same pattern as `TASK-00099`'s `test_vault_e2ee.c`) plus
...
- Round-trip: covered by `TASK-00099`'s own `test_vault_e2ee.c` (not
...
- Version DEK freshness: covered by `TASK-00099`'s test (not duplicated).
...
`TASK-00098` and `TASK-00099` per this task's acceptance criteria.
...
acceptance criteria met. Closing `TASK-00101` as `done`. This closes the
`TASK-00089` E2EE milestone (`TASK-00098`–`TASK-00101` all done) — see
