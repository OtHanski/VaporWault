...
**Out-of-domain discovery, found while implementing `TASK-00180`** (the
...
independent of `TASK-00180`'s primary-side bug and not fixed by it.

`ARCHITECTURE.md`'s "Replica hot-standby data replication" decision row
(and `TASK-00172`'s own implementation note) both describe the intended
...
- Design the correct fix: most likely, on each `replica_run_chunk_sync_pass`,
  ...
  (non-deduplicated) list — matching what `ARCHITECTURE.md`/`TASK-00172`
...
**SEC.07, 2026-08-17.** Filed during `TASK-00180`'s review — see that
task's SRV.01 implementation note and self-review for the full
investigation trail. Not fixed as part of `TASK-00180` since it's a
...
- **Manifestation confirmed live, not inert.** `vw_server_main.c` starts
  ...
  `vw_gc.c`'s `replica_lag_blocks_gc` gate (TASK-00171) only trips when
...
- **Fix**: added `vw_storage_chunk_set_refcount` (`vw_storage.c`/`.h`) —
  ...
  ARCHITECTURE.md's/TASK-00172's stated "recompute from scratch" design.
...
- Noted, not filed as a new task (severity is effectively zero, not a
  ...
  version set to ever manifest. Pre-existing since TASK-00172; this task's
  reconciliation pass inherits whatever `count` that function returns but
  doesn't change its correctness.
- Confirmed the fix doesn't reintroduce TASK-00171's replica-safety gate
