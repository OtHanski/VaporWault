...
Discovered while implementing `TASK-00207`. `notify_prefs.db` (the new
per-user notification-preference table, `vw_store.h`/`.c`) is deliberately
**not** part of `TASK-00172`'s hot-standby replication file-tag list
...
`TASK-00207` (SRV.01's task) should decide unilaterally.
...
`TASK-00173`'s client fallback, which does allow reads while degraded)
...
something to fold into `TASK-00207`'s own (non-protocol) storage work.
...
enough to fold in here, same reasoning `TASK-00214`/`00222` already used for
