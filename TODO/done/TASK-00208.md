...
blocks:      [TASK-00212, TASK-00213]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, server]
---

Implements the five admin-category triggers from `TASK-00205`'s design.
Config-only (no wire-protocol change) — reuses `TASK-00207`'s shared
...
- Same "never leak secrets into an email body" bar as `TASK-00207`.
- Do not present `replica_lag` to the admin as "a client is on
  fallback" — per `TASK-00205`'s recorded caveat, the primary has no
...
SRV.01 [2026-08-26]: Implemented. Reuses `TASK-00207`'s `vw_notify.c`
...
- Trigger call sites — each required threading a `vw_notify_ctx_t*` into
  ...
  - **replica_lag**: `vw_gc_run_once`'s existing pass-2 lag computation
    ...
    computed there for the GC-blocking decision, `TASK-00171`) — reused
  ...
  - **acme_renewal_failure**: wrapped `vw_acme_renew_if_needed`'s call
    ...
    is correct because `TASK-00205`'s own debounce list (design doc,
...
- **Security note honored**: `replica_lag`'s email body says "a replica
  ...
  caveat `TASK-00205` recorded almost verbatim.
...
- **Not covered by an automated test, disclosed rather than silently
  ...
  expected log line. Left as a real gap for `TASK-00213` to close with a
  proper automated test, same disclosure discipline as `TASK-00207`'s own
...
- The honest, checked-in note that `TASK-00205`'s own design left
