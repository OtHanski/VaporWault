...
created:     2026-08-14
priority:    high
depends_on:  [TASK-00170, TASK-00171, TASK-00172, TASK-00173, TASK-00174, TASK-00175, TASK-00176, TASK-00177]
blocks:      []
review_by:   [CQR.08]
tags:        [test]
---

Full-feature integration coverage for `TASK-00169`'s automatic-fallback
...
- **GC replica-safety regression**: pair a replica, pause/slow its pull
  ...
  regression test for `TASK-00171`'s finding, per `CLAUDE.md`'s standing
...
- Regression test for every SEC.07 finding raised during `TASK-00172`'s
  review (record-fetch handler unreachable from the normal client
  listener — the security note in that task) and `TASK-00173`/`00176`'s
...
- Sign-off notes added to `TASK-00171`/`00172`/`00173`/`00176` cross-referencing
  which test here covers it, before `ARCH.00` closes this milestone —
  same pattern `TASK-00168` already established for the multi-account
  milestone.
...
- **GC replica-safety regression**:
  ...
  caught up" half** — filed as `TASK-00180` (SRV.01): chunk refcount is
  ...
  root-cause trace is in `TASK-00180`. This test is marked
  `@pytest.mark.xfail(strict=True)` referencing `TASK-00180` rather than
...
- **Daemon automatic fallback**:
  ...
  already existed from `TASK-00174`; extended with the two things this
...
- **Gateway automatic fallback**: this task's own text named
  `test_gateway.py`, but the real fallback coverage from `TASK-00176` lives
...
- **SEC.07 regression tests**:
  ...
  (`TASK-00172`'s handler-reachability finding — sends the raw
  ...
  (`TASK-00173`/`00176`'s "fallback CA-cert never optional" findings — the
  daemon must fail closed against a real cert mismatch, the gateway must
  refuse to start at all without one).
- Sign-off notes cross-referencing the above added to `TASK-00171`, `172`,
  `173`, `176`.
...
whoever fixes `TASK-00180` doesn't need to re-read this task to know what
...
surfaced (`TASK-00180`) is properly filed rather than papered over, and the
test that found it is preserved (not deleted or weakened) so it enforces
the fix once it lands. Moving to done.

**QA.06, 2026-08-17 — follow-up.** `TASK-00180` is fixed; removed the
