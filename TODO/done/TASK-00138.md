...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127, TASK-00133, TASK-00136, TASK-00137]
blocks:      [TASK-00139, TASK-00140, TASK-00141]
review_by:   [SEC.07, CQR.08]
tags:        [web, security-sensitive]
---

Build the core file browser view against `TASK-00133`'s list/stat/mkdir/
...
Security note carried from `TASK-00127`/`TASK-00130`: this view renders
...
`TASK-00136`'s note on the no-real-browser-test gap.

**Verified against the real backend** (not mocked) via the same Node-based
`api.js` smoke test described in `TASK-00137`'s note: `listFiles`/`mkdir`/
`deleteFile` all round-trip correctly against a live gateway+server.

Not implemented in this pass (matches `TASK-00133`'s own deferral): no
