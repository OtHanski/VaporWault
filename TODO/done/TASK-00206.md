...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00205]
blocks:      [TASK-00207, TASK-00209]
review_by:   [CQR.08]
tags:        [protocol]
---

Publishes the wire side of `TASK-00205`'s user-category notification
preferences. Admin categories are server-config-only (`TASK-00208`) and do
...
- Added `VW_MSG_NOTIFY_PREFS_GET`/`_GET_RESP`/`SET`/`_SET_ACK` and the
  ...
  precedent `TASK-00197` set (enum constants published alongside the spec,
  ahead of the server/client implementation tasks) rather than leaving
  `TASK-00207`/`00209` to invent the numeric values themselves.
- No `user_id` field on either request — confirmed by design, not by
  ...
  matching `TASK-00205`'s "same trust bar as changing your own password"
  requirement.
- Storage note added to §7.13 pointing `TASK-00207` at a new `notify_prefs`
...
behavior yet (that's `TASK-00207`); nothing to unit-test at the protocol-spec
...
- `VW_NOTIFY_ALL_KNOWN` as an OR of the four defined bits is a reasonable
  ...
  `TASK-00207`'s validation check) rather than scope creep — a single macro,
