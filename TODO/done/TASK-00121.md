...
Discovered while implementing TASK-00116 (audit log filter/search/export in
...
AUDIT_QUERY acceptance criteria in TASK-00116 ("no change to the wire
...
implementing this and shipped TASK-00116 with the date/time filter deferred.
...
GUI.03 [2026-08-04]: Filed from TASK-00116. Low priority — audit log is
...
Landed together with TASK-00122's `FILE_WRITE` disambiguation as one
...
spec itself; implementation filed as TASK-00123 (SRV.01) and TASK-00124
(GUI.03), both blocked on this task and TASK-00122 reaching `done`.
...
1. §7.7 as first published implied `vw_oplog_append_raw()` — the
   ...
   bytes.
2. `TASK-00123`'s file list (as drafted) covered `vw_oplog.c`,
...
(not implementation bugs to defer to TASK-00123), since they mean the
...
the already-CRC-verified received bytes — it never re-stamps. `TASK-00123`
