...
("Sharing model", `TASK-00088`) and `docs/PROTOCOL.md` §7.5/§7.10.
...
session the same way `TASK-00131`'s session manager treats a normal login:
...
(cookies, `TASK-00131`) rather than introducing a base64 dependency for a
...
`send_file_op_error` (`TASK-00133`'s helper, reused here) had no case for
that error and fell into the catch-all "unrecognized error → evict +
500" branch built for `TASK-00155`. That catch-all is doing exactly what it
...
(the file-op error mapping was `TASK-00133`'s, but every one of `TASK-00134`'s
new handlers reuses it) — not evidence of a new server/protocol bug, and
not the same failure mode as `TASK-00155` (that one is a wire-level buffer-
size desync; this one was just a missing switch case in translation
logic). No `TASK-00155`-style connection desync was observed in any of this
task's testing.

Builds clean under both MSVC `/W4 /WX` (`build-gw-test`, Ninja) and GCC
(`build-gw-e2e`, WSL) after the fix.

Not verified: the frontend has no sharing/link UI yet (`TASK-00140`) — this
...
SEC.07/CQR.08 [2026-08-12]: Reviewed alongside `TASK-00133` (see that
...
under `TASK-00133`). This task's own endpoints (`handle_share_*`,
...
(`get_json_uint_field`), so `TASK-00133`'s unterminated-buffer finding does
