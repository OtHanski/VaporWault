...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00193, TASK-00194]
blocks:      []
review_by:   [CQR.08]
tags:        [test]
---
...
- ~~Attempting to set rules on a vault-rooted folder is rejected.~~ Struck
  — see `TASK-00193`'s correction note: no such thing as a vault-rooted
...
QA.06 [2026-08-26]: Sign-off. As with `TASK-00191`, most coverage was
written and verified incrementally as part of `TASK-00193` itself (real
...
- `test_cli_selective_sync_exclude_stops_further_uploads` (written on
  `TASK-00193`): excluded file's local edits never reach the server
...
- `test_cli_selective_sync_excluded_remote_change_never_redownloaded`
  ...
  the server. This is exactly the failure mode `TASK-00193`'s own
...
- `test_cli_selective_sync_rules_persist_across_daemon_restart` (new):
  ...
  call sites `TASK-00193` had to update by hand.
- Vault-rooted-folder rejection: dropped, not silently skipped — see the
  struck acceptance-criterion line above, matching `TASK-00193`'s already-
...
(up from 104 after `TASK-00190`, +3 for these tests +... — the daemon-
...
regressions. Sign-off given for `TASK-00192`'s milestone — moving to
`done`.
