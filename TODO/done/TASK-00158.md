...
203). This was real, working evidence used to justify `TASK-00156`'s own
...
PRT.04 [2026-08-12]: Filed while implementing `TASK-00156` — GUI adoption
...
`TASK-00156`'s new wire field only reached as far as `vw_client_core.c`'s
...
- `vw_cache_entry_t` (`vw_cache.h`) gains a `vault_id` field. This is an
  ...
  - Verified per this session's standing discipline: added a regression
    test (`tests/unit/test_vw_sync.c`, "vw_cache_open: pre-TASK-00158
...
- `srv_entry_t` (`vw_sync_internal.h`) and `srv_push()` (`vw_sync.c`) gain
  ...
  `TASK-00156` already populates from the wire.
