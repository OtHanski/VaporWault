...
Discovered while writing `TASK-00199`'s `test_cli_search.py`: creating a new
...
(`TASK-00113`: a directory that already exists *on the server* gets
...
implementing `TASK-00199`. Worked around in `test_cli_search.py` by not
...
ever ran for a SHARED folder's new file (`TASK-00113`) — an OWNED folder's
