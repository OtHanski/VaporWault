...
- Daemon IPC (`vw_ipc.h`, matching `cmd_create_link`'s existing IPC
  ...
  list-links response (mirrors `TASK-00187`'s `LINK_LIST_RESP` addition).
...
- `create-link /foo.txt view 0 --password secret` mints a link that
  `LINK_ACCESS` (tested via `TASK-00191`) actually enforces.
...
- `vw_client_core.h`/`.c`: `vw_client_link_create` gains a `password`
  parameter (sent as an optional trailing string, matches `TASK-00186`'s
...
- Fixed two other call sites the signature change broke: the web
  ...
  `TASK-00190`'s job, noted inline) and two unit tests' now-stale
  `vw_share_link_create` call sites were already fixed on `TASK-00187`.
- Verified: `build-msvc-105` and `build-gw-e2e` (WSL/GCC) both build
  ...
  101 after `TASK-00187`), zero regressions.
