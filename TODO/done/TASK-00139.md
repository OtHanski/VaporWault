...
created:     2026-08-10
priority:    normal
depends_on:  [TASK-00127, TASK-00133, TASK-00138]
blocks:      []
review_by:   [CQR.08]
tags:        [web]
---

Build upload/download against `TASK-00133`'s chunk-level endpoints, driving
the `CHUNK_QUERY`→`CHUNK_UPLOAD`→`FILE_COMMIT` (upload) and
`VERSION_CHUNKS`→`CHUNK_DOWNLOAD_REQ` (download) steps directly from the
browser, one HTTP request per chunk step — per `TASK-00127`'s note, this gives
...
endpoints that `TASK-00133` had deferred — implemented those first (see
`TASK-00133`'s addendum note: `/api/chunks/upload`, `/api/files/commit`,
...
- *A dropped connection mid-upload fails cleanly with a retry option*:
  ...
  no-browser-available caveat as `TASK-00136`-`138`), but the underlying
...
`try`/`catch` (unlike several other handlers reviewed under `TASK-00137`/
