...
Discovered while implementing `TASK-00207` (user-facing email alerts —
`account_security_change` is supposed to fire on "password changed, or
2FA enabled/disabled" per `TASK-00205`'s design). Checked thoroughly before
...
`TASK-00207` was implemented against reality rather than the design's
...
honestly in `TASK-00207`'s own notes rather than the trigger being
silently left half-implemented with no explanation, or this task's actual
missing-feature scope being quietly folded into `TASK-00207` itself (a
...
- Once designed, file the PRT.04 (protocol spec) and SRV.01 (server
  handler) follow-up tasks, and connect `TASK-00207`'s
...
while implementing `TASK-00207`'s notification dispatch).
...
`CQR.08`, the same routing-rule-1 gap `TASK-00222` also had; added
`SEC.07`.
...
- **New precondition found during design, not anticipated by this
  ...
  `TASK-00222` shipped the same day) would lock that account out of every
...
Full implementation, done directly (see `TASK-00222`/`00223`'s same
...
wired end to end, unlike `TASK-00223` where GUI/web were left as a
...
a successful toggle, closing the gap `TASK-00207` explicitly left open in
its own notes.
...
when `TASK-00213` shipped (2FA toggle turned out to be a much simpler real
...
- Verified the check order in `handle_account_2fa_set`: password
  ...
  password. No enumeration surface here, unlike `TASK-00222`'s disclosed
...
`_by_id`-style client-core refactor pattern from `TASK-00223` was not
...
`TASK-00222`'s own edit) forbids it for email specifically. The CLI's
