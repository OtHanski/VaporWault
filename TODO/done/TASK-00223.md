...
`TASK-00214` corrected a wrong assumption from `TASK-00182`: the server's
...
- Wire the new client-core function(s) through daemon IPC
  ...
  connection pattern (see `TASK-00209`'s note on this — each IPC request
...
PRT.04 [2026-08-27]: Filed as `TASK-00214`'s promised follow-up after
determining the server side needed only a one-line permissive relaxation
(landed directly in `TASK-00214`, not deferred here) — see
...
- **Client core** (`vw_client_core.h`/`.c`): refactored
  ...
  `TASK-00095`/`00106`). The `_by_id` restore sends `virtual_path=""` per
  `TASK-00214`'s relaxation; no wire change needed, exactly as `TASK-00214`
  predicted.
