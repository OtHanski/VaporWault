...
**Out-of-domain discovery, found while implementing `TASK-00178`** (the
...
cluster/fallback milestone `TASK-00178` was testing.
...
delete+GC — never that the chunk itself is actually collected. `TASK-00171`'s
own unit test (`tests/unit/test_vw_gc.c`) exercises the replica-lag gating
logic directly against `vw_storage_t`, which (per `TASK-00172`'s own note)
...
`TASK-00178`'s new `test_gc_does_not_delete_chunk_while_replica_lags`
...
- Check `vw_storage_chunk_put_replicated`'s call site
  ...
  issue on the replica side — TASK-00172's design note says the replica
...
- Regression test: extend or add to `tests/unit/test_vw_gc.c` and/or
  ...
  `test_gc_does_not_delete_chunk_while_replica_lags` (`TASK-00178`) already
...
**QA.06, 2026-08-17.** Filed while implementing `TASK-00178`'s GC
...
deserves its own review rather than a same-task patch. `TASK-00178` itself
...
(`vw_cluster.c`) and confirmed `TASK-00172`'s design-note claim ("sets its
...
**Verification:**
- `tests/integration/test_cluster.py::test_gc_does_not_delete_chunk_while_replica_lags`
  (`TASK-00178`'s own regression test for this exact bug) now genuinely
  passes — `xfail(strict=True)` marker removed (by QA.06, see `TASK-00178`'s
  own follow-up note).
...
under-count noted as real but separate — filing as `TASK-00181` (SRV.01)
rather than leaving it as a comment-only flag, per SEC.07's own
recommendation above. Moving TASK-00180 to done.
