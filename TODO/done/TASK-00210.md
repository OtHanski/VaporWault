...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00209]
blocks:      []
review_by:   [CQR.08]
tags:        [gui]
---

Blocked on `TASK-00209` (GUI consumes client/daemon API only).
...
- `vw_gui_ipc.h`/`.cpp`: `VwGuiIpc::notify_prefs_get`/`_set`, each opening
  ...
  the same thing `TASK-00209` had to fix its own CLI code for).
...
- **Found and filed, not fixed here** (`TASK-00221`): the Settings view's
...
- **Round-trips through the daemon and reflects real server state, not
  ...
  messages `TASK-00209`'s `vapourwault-cli notify` already exercises
  ...
  `TASK-00200`'s search-bar GUI work) — what was actually verified instead:
  ...
  verified, same caveat `TASK-00200`/`TASK-00216` recorded for their own GUI
  work.
...
- The checkbox handler updating local state from the ACK's echoed value
  ...
  elsewhere (e.g. `TASK-00207`'s own SET_ACK echo-back rationale).
  `NOTIFY_CATEGORIES[]` mirroring the CLI's own table by name/bit is a
  good anti-drift choice, same reasoning `TASK-00209` used for its own copy.
...
- `TASK-00221`'s filing is honest and correctly scoped — a real,
