---
id:          TASK-00150
title:       "Linux .rpm packages for server & client (CPack RPM, reusing TASK-00149 scripts)"
status:      done
assignee:    BLD.05
created_by:  ARCH.00
created:     2026-08-11
priority:    normal
depends_on:  [TASK-00145, TASK-00146, TASK-00149]
blocks:      []
...
generator, reusing `TASK-00149`'s `packaging/linux/scripts/*.sh` content
...
2. Wire `CPACK_RPM_<COMPONENT>_POST_INSTALL_SCRIPT_FILE`,
   ...
   `TASK-00149`'s scripts directly where the content is identical (the
...
4. RPM has no separate "purge" concept (no config/data distinction
   ...
   (`TASK-00154`): `dnf remove` on RPM behaves like DEB's `purge`, not its
...
- **Not verified end-to-end in this pass** — same caveat as `TASK-00149`;
  `TASK-00152`'s job on a real `dnf`-based system.
...
design's initial implementation wave. Normal priority (vs. `TASK-00149`'s
...
against the produced `.rpm` files — same real-install rigor as `TASK-00149`'s
DEB testing, not just a build-succeeds check).

**A real design mistake caught and fixed while implementing this**: the
original plan (recorded in `TASK-00149`'s design note) was to factor the
...
(TASK-00149, DEB) and `packaging/linux/scripts/rpm/server-postun.sh` (this
...
{server,client}/{postinst,prerm}` directly (unchanged from TASK-00149) —
...
- Fresh install: service user, directories, config template — identical
  results to `TASK-00149`'s DEB test.
...
- **Final removal** (`dnf remove` after the upgrade, so `$1 == 0`):
  ...
  real, not a scripting bug — `TASK-00154` must document it plainly.
...
(`TASK-00149`'s test) showed no such warning for the identical `useradd`
invocation. Not fixed in this pass since it's non-fatal; flagged for
`TASK-00153`/`TASK-00154` to decide whether to silence it (e.g. resolve the
...
`TASK-00149`, and the documented DEB/RPM removal-semantics asymmetry
