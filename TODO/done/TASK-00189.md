...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00188]
blocks:      [TASK-00191]
review_by:   [CQR.08]
tags:        [gui]
---

Blocked on `TASK-00188` (GUI consumes client/daemon API only).
...
- `vw_view_browser.cpp`'s `render_share_dialog` (the actual home of the
  "Create link" UI, per `TASK-00189`'s own correction note above): a
...
- Verified: `build-msvc-105` builds and links clean (zero warnings),
  `ctest` 18/18. Same caveat as `TASK-00183`: no interactive/rendered
