...
created:     2026-08-25
priority:    normal
depends_on:  [TASK-00186]
blocks:      [TASK-00188, TASK-00190, TASK-00191]
review_by:   [SEC.07, CQR.08]
tags:        [security-sensitive, server]
---

Implements `TASK-00186`'s wire spec server-side.
...
- This is a new externally-reachable input surface via the web gateway
  (`TASK-00190`) — coordinate with SEC.07 before this reaches `done`.
...
- `vw_share.h`: `link_password_hash[32]`/`has_link_password` carved from
  ...
  `link_token` via HMAC-SHA256 (see the design note on `TASK-00186` and
...
- New integration coverage (`tests/integration/test_link_password.py`,
  ...
  `has_password`) — this is most of `TASK-00191`'s eventual scope, written
  now while the wire format was fresh in context rather than deferred;
  `TASK-00191` will still need the CLI/GUI/gateway end-to-end passes once
  `TASK-00188`-`190` land.
...
- Verified: `build-msvc-105` (MSVC) and `build-gw-e2e` (WSL/GCC, rebuilt
  after `TASK-00215`'s stale-`VW_WERROR` fix) both build clean, zero
