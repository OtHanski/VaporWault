...
created:     2026-08-13
priority:    normal
depends_on:  [TASK-00161, TASK-00165]
blocks:      [TASK-00168]
review_by:   [CQR.08]
tags:        [build, docs]
---

Depends on `TASK-00161`'s daemon `accounts/<id>/` state layout and
`TASK-00165`'s new gateway `--state-dir`. `docs/DEPLOYMENT.md` and the
...
- Document the new gateway `--state-dir` flag: where it should live per
  ...
  `TASK-00165`'s security note rather than re-deriving it.
- `packaging/linux/scripts/` (systemd unit files, postinst/prerm/postrm
  ...
  (`Install-VaporWault*.ps1` / WiX fragments, per `TASK-00145`'s existing
...
- A fresh install (per platform) ends up with the gateway's state
  ...
  bits matching `TASK-00165`'s requirement — verified, not just documented.
...
protection — nothing TASK-00161-specific to add there.
...
- `packaging/linux/vapourwault-web-gateway.service`: added `--state-dir
  ...
  own comment, which pre-`TASK-00165` correctly said the gateway "has no
...
- `docs/DEPLOYMENT.md`: new §11.7 documenting both the daemon's
  ...
  referencing `TASK-00165`'s security rationale rather than re-deriving it,
...
- No C/CMake changes in this task; both build trees and the full test suite
  were already green from `TASK-00166` and untouched here.
