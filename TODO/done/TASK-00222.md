...
Discovered while implementing `TASK-00213` (integration tests for the
...
- `USER_MODIFY`/`_ACK` (`0x0603`/`0x0604`) is a reserved opcode with
  **no handler implemented anywhere** — confirmed by grepping
  `vw_admin.c`/`vw_file_handlers.c` for it, same finding `TASK-00219`
...
`AUTH_RECOVER_REQUEST`, shipped in `TASK-00046`) and by `vw_notify.c`'s
`notify_send_if_enabled` (`rec.email[0] == '\0'` short-circuits every
trigger, `TASK-00207`). Since no account created through either existing
...
- Decide the actual mechanism (this needs a real design call, not a
  ...
  - And/or design and implement `USER_MODIFY` for real (admin-driven
    ...
    same design question `TASK-00219` raised for 2FA, worth deciding
...
something QA.06 should invent unilaterally while writing `TASK-00213`'s
...
codebase. Marked `priority: high` rather than `low` (unlike `TASK-00219`,
its closest sibling finding) because this doesn't just block a new
feature — it silently breaks an already-shipped one (`TASK-00046` password
...
- **Not** overloading `USER_MODIFY` (`0603`/`0604`) as a generic
  field-patch message: `TASK-00219` independently reached the same
...
- **Server** (`src/server/vw_store.h`/`.c`): new `vw_email_validate()` —
  ...
  TASK-00157) — copied to `email_ht_hole_on_probe_path`/`_remove_at`/
...
- **Tests**:
  ...
  pointing at the new tests instead, since TASK-00222's own acceptance
...
`path_ht_remove`/`_remove_at`/`_hole_on_probe_path` (TASK-00157) — same
