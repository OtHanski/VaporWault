...
created:     2026-08-13
priority:    normal
depends_on:  [TASK-00161]
blocks:      [TASK-00168]
review_by:   [CQR.08]
tags:        [client]
---

Depends on `TASK-00161`'s daemon `ACCOUNT_*` IPC messages and account-scoped
...
- New subcommands: `vapourwault-cli account add`, `account list`,
  ...
  - **`account add` takes host/port/CA-cert per account, not just
    ...
    [otp]` syntax (this task's starting point, added by `TASK-00161` just to
...
## Out-of-domain discovery (TASK-00161)
...
TASK-00161's new layout those fields no longer exist on `daemon.conf` (they
...
not fixed by TASK-00161 itself (out-of-domain per `CLAUDE.md`'s routing
rule).
...
replace the old `login` command outright (`login` was TASK-00161's own
...
either of `TASK-00161`'s own tests, since neither exercised the actual CLI
...
vw_gui_ipc.cpp`, written by `TASK-00161`) for the same pattern: currently
...
since `TASK-00163` is about to rewrite that same call site to add CA-cert
support anyway; noted in `TASK-00163` to fix it there instead of here.
