...
created:     2026-07-10
priority:    high
depends_on:  [TASK-00021, TASK-00008]
blocks:      [TASK-00023, TASK-00024]
...
TASK-00023 (`vw_storage_files`). TASK-00021 must be done before this task starts so the
...
- `vw_store_version_create(vw_store_t *s, const vw_version_record_t *rec, const uint8_t *chunk_hashes, uint64_t *out_version_id)`
  ...
  Ref-count increment for each chunk hash is the caller's responsibility (TASK-00023).
...
regardless of owner — the caller (vw_server_core / TASK-00024) must verify `rec.owner_id
...
Path traversal prevention is at the protocol spec level (TASK-00021 §6) and enforced by
...
**Deviations from TASK-00022 spec:**
...
- `vw_file_store_t` is a separate opaque context (not embedded in `vw_store_t`).
  Callers (TASK-00024) hold both a `vw_store_t *` and a `vw_file_store_t *`.

**Security notes** (to be audited by SEC.07):

- `vw_store_file_get_by_path`: explicitly does NOT check `owner_id` against session;
  callers (TASK-00024) must enforce this per ARCH.00 note above.
...
CQR.08 [2026-07-11]: Code reviewed against TASK-00022 acceptance criteria.
...
by design. TASK-00024 must verify `rec.owner_id == session.user_id` before any wire
response (already tracked in TASK-00021 SEC.07-A-2).

**SEC.07-B-2 (advisory)**: `vw_store_version_get_chunks` returns a caller-freed buffer
of chunk hashes. TASK-00024 must not expose these hashes to sessions that do not own the
file — to be audited in TASK-00024.
...
Sign-off granted. TASK-00022 may move to `done` after ARCH.00 confirmation.

---

ARCH.00 [2026-07-11]: TASK-00022 closed. CQR.08 and SEC.07 signed off; no blocking
...
GC backlog. TASK-00023 (chunk content store) and TASK-00024 (server request handlers) are
