...
handshake, per the spec published in TASK-00047.
...
7. On success: send `NODE_HELLO_OK`, then hand the connection to the
   replication loop (TASK-00049 will implement this; for now, the connection
...
- One advisory: `cluster_log` writes to stderr; in production the main logging should be integrated (already noted as a TASK-00049 concern). Not blocking.
...
ARCH.00 [2026-07-13]: Both reviewers signed off, no blocking findings. TASK-00048 done. TASK-00049 is now unblocked.
