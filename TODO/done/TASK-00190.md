...
this is the highest-traffic consumer of `TASK-00185`'s design.
...
- Frontend redemption page (the page an anonymous visitor lands on via a
  ...
  design — see `TASK-00185`'s correction note) — don't invent a distinct
...
the real gap as `TASK-00216` (building the redemption UI itself is a
...
yet, since `TASK-00216` will need exactly this passthrough once it lands.
...
- `vw_gateway_api.c`: `handle_link_create` reads an optional `password`
  JSON field (replacing the `TASK-00188`-era `NULL` placeholder);
...
- Verified: `build-msvc-105` and `build-gw-e2e` (WSL/GCC) both build
  ...
  after `TASK-00188`), zero regressions.
...
- No blocking findings. Signing off; `TASK-00216`'s own security review
