...
- `ci.yml` always builds with `VW_BUILD_GUI=OFF` (QA.06 flagged this exact gap
  during `TASK-00081`'s review, tracked as a `TASK-00082` follow-up).
...
- **Linux**: WSL running Ubuntu 24.04 — the exact same OS as the `ubuntu-latest`
  ...
  `TASK-00084`) plus one CMake wiring gap directly in this task's scope: `src/gui/
  ...
  fixes plus `TASK-00084`'s app-code fixes, a full `cmake --build` succeeded:
...
- **Windows**: local MSVC BuildTools 2022 (17.14, `cl` 19.44.35207) — vendored
  ...
  initially hit the same `TASK-00084` app-code bugs (MSVC additionally caught one
  ...
  `TASK-00084`'s "separately noted" section for the full reasoning). After all
  `TASK-00084` fixes, the only remaining local failure was that unrelated
...
`TASK-00084` before pushing the fix and re-running `release.yml`.
...
the first `NewFrame()`/render call. All other `TASK-00084` fixes verified
...
ARCH.00 [2026-07-21]: CQR.08 signed off clean on both `TASK-00083` and
`TASK-00084`. Closing — status: done. Proceeding to commit, move the `v0.1.0` tag
