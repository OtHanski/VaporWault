...
key or passphrase** — that happens entirely in the browser (`TASK-00141`),
per `TASK-00127`'s design decision preserving the vault's zero-knowledge
property (`ARCHITECTURE.md`'s E2EE model, `TASK-00089`).
...
- Structural verification (not just behavioral): no code path in this
  ...
  nothing to receive by construction. `TASK-00143` adds the corresponding
...
as `link_token`, `TASK-00134`) — capped at `VW_GATEWAY_MAX_WRAPPED_VK_BYTES`
...
Shares the `send_file_op_error` helper with `TASK-00133`/`TASK-00134` — the
`VW_ERR_AUTH_REQUIRED` mapping fix documented in `TASK-00134`'s note applies
...
Not implemented/verified: no frontend vault UI yet (`TASK-00141`, the
