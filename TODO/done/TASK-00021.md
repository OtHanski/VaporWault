...
priority:    critical
depends_on:  []
blocks:      [TASK-00022, TASK-00024, TASK-00025]
...
transfer operations. SRV.01 (TASK-00022/00023/00024) and CLI.02 (TASK-00025) are blocked on this
...
task — the constants are needed by TASK-00022/00024/00025 compile guards.
...
4. `VW_MSG_VERSION_CHUNKS = 0x0305`, `VW_MSG_VERSION_CHUNKS_RESP = 0x0306` added to
   `vw_msg_type_t` (gap identified by CLI.02 in TASK-00025).
...
**Design decisions:**
- Kept existing 0x02xx/0x03xx message numbering (supersedes TASK-00021's proposed 0x01xx
...
- VERSION_CHUNKS added to resolve the download gap in TASK-00025 §5 before that task
...
Good defensive choice. Implementation (TASK-00024) must match exactly — a `VW_ERR_PERMISSION`
...
other processing". Implementation (TASK-00024) must not parse any subsequent payload fields
...
Tag TASK-00024 as `security-sensitive` (already done by ARCH.00).
...
allocating a zero-length buffer and reading from it. Low risk but document in TASK-00024.

Sign-off: spec is sound. All file-transfer security properties are correctly specified.
Implementation tasks (TASK-00024 security review will catch implementation deviations).
...
the encode/decode implementor (TASK-00024) to document the wire vs. in-memory layout
distinction clearly.
...
Suggest the TASK-00024/00025 implementor add an explicit `uint16_t path_len` + `const char *path`
...
complete and matches the header. TASK-00021 may move to `done`.

---

ARCH.00 [2026-07-11]: TASK-00021 closed. SEC.07 and CQR.08 signed off; no blocking findings.
Advisories A-1/A-2/A-3 from SEC.07 and A-1/A-2 from CQR.08 are routed to TASK-00024/00025 as
implementation notes. TASK-00022 and TASK-00025 are now unblocked.
