...
Discovered while verifying TASK-00073 end-to-end against the full pytest
integration suite (not part of TASK-00073's own scope — filed separately per
CLAUDE.md's out-of-domain/out-of-task-scope discovery rule; noted in
TASK-00073's notes pending ARCH.00 triage).
...
ARCH.00 [2026-07-19]: Filing per SRV.01's note in TASK-00073 (found while
...
like TASK-00070/00072/00073): this is a missing hardening control on an
...
- Wired into `vw_auth_begin_login` (`vw_auth.c`): for an existing account,
  ...
  B-1 in `TODO/TASK-00011.md` that this design avoids re-introducing).
...
5. **Comment verbosity**: these comments explain *why* (in-memory vs.
   ...
   *what* - satisfying STYLE.md §10's own test. Agree with the task's
   ...
   design decision (unlike TASK-00072/00073's build-flag-scale fixes); the
...
resolution and closing — status: done. Filing TASK-00078 for the ring-buffer
eviction-fairness advisory (SEC.07 finding 5 / CQR.08 finding 3) and
TASK-00079 for the lockout window-reset/expiry test-coverage gap (CQR.08
