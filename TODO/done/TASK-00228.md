...
created:     2026-08-31
priority:    high
depends_on:  [TASK-00226, TASK-00227]
blocks:      [TASK-00229]
review_by:   [CQR.08]
tags:        []
---
...
- Progress reporting hook for the UI (TASK-00229) to bind to.
...
- MOB.10, 2026-09-02: Implemented under a new `transfer` package:
  ...
  progress/completion hook TASK-00229's UI binds to).
  **Deliberate deviation from this task's own scope text**: skipped the
  ...
  redundant full-file copy) specifically because TASK-00226's chunk-level
  ...
  which Android doesn't use at all (TASK-00226 chose the raw primitives
...
- **Runtime verified, 2026-09-02**, against a real `vapourwaultd` (a
  ...
  gap as `TASK-00225`/`00226`'s discoveries, folded into `TASK-00237`, not a
  ...
  - Added a `transferTestButton` to the existing throwaway test Activity
    ...
    flow itself was not exercised here** — that's TASK-00229's UI to
    build; what TASK-00228 owns and what this verifies is the transfer
...
called out clearly in the API docs consulted beforehand):
  ...
  - No other blocking findings. Thread-safety of `PendingWork`/
    ...
    `rewind()`-not-`flip()` discipline from TASK-00226's own lesson, both
    reviewed clean.
