...
Discovered while writing `TASK-00143`'s integration tests for the web
...
- File-id-based operations (`vw_client_file_stat_by_id`,
  `vw_client_file_download_by_id`, etc., TASK-00095) are unaffected — they
  ...
  `TASK-00138`, navigates and re-stats by path routinely) and for the
...
- A regression test covering mkdir → move → stat-by-new-path, added to
  ...
  `tests/integration` (`TASK-00143`'s `test_gateway.py` has a version-list
  ...
  suite, not just new `TASK-00143` work).
...
WEB.09 [2026-08-12]: Filed while writing `TASK-00143`'s integration tests —
...
filing this. Worked around in `TASK-00143`'s own test suite by not
...
- Re-enabled the stat-after-move assertions in `TASK-00143`'s
...
**Other findings, on `tests/integration/test_gateway.py` (TASK-00143's
suite, since TASK-00157 touched the same test-move path)**:
...
Ready for ARCH.00 to move to `done` alongside `TASK-00143`.
