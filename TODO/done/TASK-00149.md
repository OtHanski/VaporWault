...
2. New `packaging/linux/scripts/` — shared portable POSIX-`sh` logic,
   invoked as DEB scriptlets (this task) and RPM scriptlets (`TASK-00150`),
   per `TASK-00145`'s "write once, thin per-ecosystem wrapper" design:
...
- Maintainer scripts pass `shellcheck` (or equivalent) with no warnings
  ...
  `TASK-00081` required for `release.yml`'s shell steps.
- **Not verified end-to-end in this pass** (no Linux environment available
  ...
  confirm real behavior) — `TASK-00152`'s job, and should block this task's
...
BLD.05 [2026-08-11]: Implemented and — unlike `TASK-00147`/`TASK-00148` — fully
...
generator ignores it for this specific aspect" surprise as `TASK-00146`'s
...
**Update, `TASK-00152`**: `shellcheck` has since been run for real
(`TASK-00153`) — clean, zero findings. The `systemctl --user` startup gap
