...
created:     2026-08-10
priority:    high
depends_on:  [TASK-00127, TASK-00131, TASK-00132]
blocks:      [TASK-00138, TASK-00139]
...
logged-in session's `vw_client_sess_t` from `TASK-00131`.
...
- Chunk-level upload: expose the protocol's own
  ...
  `TASK-00127`'s note) without inventing a new status/progress API.
...
- Chunk upload/download endpoints correctly surface partial progress to the
  browser (verified in `TASK-00139`).
...
as `TASK-00155` (assigned `PRT.04`, out of this role's domain per
...
WEB.09 [2026-08-11]: Addendum found while implementing `TASK-00134`:
...
revoked mid-lifetime, `TASK-00134`) — and fell into the catch-all "unrecognized
error → evict + 500" branch built above for `TASK-00155`. Added an explicit
...
scoped-session case `TASK-00134` exercised) — see `TASK-00134`'s note for the
...
implementing `TASK-00139`'s backend prerequisite. Added to
...
`wrapped_dek` for the vault-download path `TASK-00141` will need, over
...
WEB.09 [2026-08-12]: Second addendum, found while implementing `TASK-00141`:
...
survives, subsequent requests on it succeed normally. See `TASK-00141`'s
note for the full repro.

Moving to `review` — needs SEC.07 + CQR.08 sign-off. `TASK-00155`'s
...
return code. `vw_json_string_decode` (`TASK-00130`) could return an error
*after* writing a partial, non-empty prefix and *without*
NUL-terminating it (that module's own bug, fixed under `TASK-00130`'s own
...
itself (`TASK-00130`'s note has the detail — it now always leaves
