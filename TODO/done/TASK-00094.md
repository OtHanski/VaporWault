...
`docs/PROTOCOL.md` §7.5/§7.10 (`TASK-00088`). This is the security-critical
...
- Extend `vw_session_record_t` (`vw_store.h`) with `scope_file_id`/
  ...
  reuse, matching `TASK-00090`'s `deleted_at` precedent.
...
- **`FILE_LIST_RESP`/`FILE_STAT_RESP` permission field**: stop hardcoding
  ...
  `vw_perm_t` value). `TASK-00096`'s file-browser indicator reads this field.
...
ARCH.00 [2026-07-29]: Filed as part of decomposing `TASK-00088` once its
...
`rate_entry_t` pattern, with TASK-00078's eviction-fairness fix applied from
the start).

**`vw_session_record_t` extended** (`vw_store.h`): new `scope_share_id`
field, reusing `_pad[70]` (same precedent as TASK-00090's `deleted_at`).
...
writing this task's own integration tests): `TASK-00104` (PRT.04 — no wire
message creates a directory at all, a general Phase 2 gap predating
sharing) and `TASK-00105` (SRV.01, low priority — an unrecognized message
type on an authenticated connection hangs silently instead of erroring).
`TASK-00104` in particular constrained this task's own integration-test
...
- New `tests/integration/test_sharing.py` (12 tests) against a real
  ...
  discovered the hard way — via TASK-00105's finding — that exceeding the
...
client's sharing support is `TASK-00095`, not started yet. `TASK-00097`'s
comprehensive scenario matrix (this task's own tests are TASK-00094's
...
protocol client rather than `vapourwault-cli`, since `TASK-00095` (the real
...
`TASK-00095`/`TASK-00096` are now unblocked; `TASK-00097` should build on this
task's `vw_client.py` extensions rather than re-inventing them.

QA.06 [2026-07-31]: `TASK-00097` closed — extended `test_sharing.py` (this
task's own suite) to the full regression matrix, building on the
`vw_client.py` extensions from here as anticipated above. See `TASK-00097`
