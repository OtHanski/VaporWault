...
Each node entry (fixed 158 bytes; corrected here `TASK-00285` — this table previously omitted the `role` byte the implementation always sent and described `hostname` as a length-prefixed string when it has always been a fixed-size, NUL-padded field):

| Field              | Type          | Notes |
|--------------------|---------------|-------|
| node_id            | uint64        | `auth_token` intentionally omitted — never included in this response |
| is_active          | uint8         | 1 = currently connected and replicating |
| role               | uint8         | `VW_NODE_ROLE_REPLICA` (0) or `VW_NODE_ROLE_SELF` (1) — this entry's own role value, not the responding server's overall role (that's the top-level `role` field above) |
...
| hostname           | uint8[128]    | Human-readable label, NUL-padded (not length-prefixed) |
...
**Implementation note (`TASK-00285`):** the pre-`TASK-00284` implementation only sent OPLOG_ACK (and therefore only updated `sync_watermark`/`lag_entries`) when a poll actually had new oplog entries to acknowledge — skipped entirely on an empty/caught-up poll, since there was nothing new to acknowledge either way. That skip would have silently defeated `client_conn_count`'s "bounded by the poll interval" staleness claim above the moment it started riding the same exchange: a replica with no new primary activity, but a real client connected to it, would never get a chance to report that fact. Fixed alongside this field's implementation: both sides now always exchange OPLOG_ACK every poll cycle, re-acknowledging the unchanged watermark on an empty batch, so `client_conn_count` genuinely stays within one `replica_poll_interval_secs` of current regardless of primary write activity. No wire format change from this fix — `OPLOG_ACK`'s byte layout is unchanged, only when it's sent.
