...
| `notify.chunk_unrepairable` | 0/1 | `0` | Alert when the background scrub pass or a client download hits a corrupt chunk that the repair pipeline cannot fix — both local Reed-Solomon reconstruction and every reachable replica were tried and failed. Debounced per chunk hash, not a threshold: fires once per distinct corrupt chunk per server run — a client repeatedly retrying the download, or the periodic scrub pass re-finding the same still-broken chunk, does not re-alert. Not persisted; a restart re-arms every hash. |
