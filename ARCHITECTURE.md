...
### Android client modules (`TASK-224` design; `vw_jni_bridge`/`VwClient` implemented `TASK-225`/`226`; core browse/login/transfer UI done `TASK-229` — vault support still TASK-230, sharing/account-self-service UI still TASK-231/232)
...
| Transfer service | `android/app/src/main/java/.../transfer/` | MOB.10 | Kotlin | SAF-based on-demand upload/download, streamed directly (no private-cache staging — TASK-228 found this unnecessary given TASK-226's chunk-level primitives); runs as a User-Initiated Data Transfer job / foreground-service fallback — done, `TASK-228` |
| UI (Views) | `android/app/src/main/java/.../ui/` | MOB.10 | Kotlin | File browser (breadcrumb + CRUD), login/2FA challenge, transfer queue — done, `TASK-229` (full 2FA login success blocked on the server-side bug filed as `TASK-238`, not on this UI). Account/profile, sharing/links, vault create/unlock/browse still pending (`TASK-230`-`232`). |
