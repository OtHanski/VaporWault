...
### Android client modules (`TASK-224` design; `vw_jni_bridge`/`VwClient` implemented `TASK-225`/`226`; core browse/login/transfer UI done `TASK-229`; vault support done `TASK-230` — sharing/account-self-service UI still TASK-231/232)

| Module | File(s) | Owner | Language | Responsibility |
|--------|---------|-------|----------|----------------|
| `vw_jni_bridge` | `android/app/src/main/cpp/vw_jni_bridge.{h,c}` | MOB.10 | C | JNI shim over `vw_client_core` (session lifecycle, file ops/chunking, sharing, version history, account self-service — `TASK-225`/`226`) and `vw_vault` (setup/unlock/close/list/upload/download — `TASK-230`), all done. Also owns the `JNI_OnLoad` hook that calls `vw_crypto_init()` — required before any vault operation's `vw_crypto_random`/Argon2id call, the Android analogue of `vw_daemon.c`'s own startup call (`TASK-230` found this was missing entirely). |
| `VwClient`/`VwNative` | `android/app/src/main/java/.../{VwClient,VwNative}.kt` | MOB.10 | Kotlin | `VwNative`: raw 1:1 JNI mirror. `VwClient`: the ergonomic wrapper — mobile analogue of desktop's `VwGuiIpc`, minus the socket (calls straight into linked-in native code). Done (`TASK-226`); `VwClient.listVaults()` added in `TASK-230`. |
| `VwVault` | `android/app/src/main/java/.../VwVault.kt` | MOB.10 | Kotlin | Ergonomic wrapper over an unlocked vault handle — `setup`/`unlock`/`close`/`uploadFile`/`downloadFile`. Passphrase is `CharArray`-only end to end (never a JVM `String`) — done, `TASK-230`. |
...
| Transfer service | `android/app/src/main/java/.../transfer/` | MOB.10 | Kotlin | SAF-based on-demand upload/download, streamed directly (no private-cache staging — TASK-228 found this unnecessary given TASK-226's chunk-level primitives); runs as a User-Initiated Data Transfer job / foreground-service fallback — done, `TASK-228`. `VaultTransferer` (`TASK-230`) is the vault-aware counterpart — it *does* stage through the private cache, since `vw_vault_upload_file`/`_download_file` are whole-file native calls needing a real POSIX path, unlike the plaintext chunk-streaming path. |
| UI (Views) | `android/app/src/main/java/.../ui/` | MOB.10 | Kotlin | File browser (breadcrumb + CRUD, plus a vault mode — `TASK-230`), login/2FA challenge, transfer queue, vault create/unlock/browse (`VaultActivity`) — all done (`TASK-229`/`230`; full 2FA login success blocked on the server-side bug filed as `TASK-238`, not on this UI). Account/profile self-service and sharing/links UI still pending (`TASK-231`/`232`). |
