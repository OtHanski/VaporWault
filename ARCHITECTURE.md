...
| Selective-sync rules | Per-sync-folder glob-style exclude patterns, stored on `vw_sync_folder_t`/`account.conf`, enforced entirely client-side in `vw_sync.c`'s BFS collect step — no server/protocol change. Excluding an already-locally-synced path never deletes the local copy; it only stops pulling further remote changes into it. Not available on vault-rooted sync folders | Corrects a fictional claim: this document previously attributed "selective-sync rules" to `vw_cache`, but no such code, in `vw_cache` or anywhere else, ever existed (flagged by the 2026-08-05 audit note, designed for real 2026-08-25, `TASK-00192`–`195`) |
